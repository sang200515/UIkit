//
//  FBeautyReportViewController.swift
//  fptshop
//
...
class FBeautyReportViewController: UIViewController, MFMailComposeViewControllerDelegate, UISearchBarDelegate {
    ...
    var listDSRealtimeMyPham: [MyPhamRealtime] = []
    ...
    override func viewDidLoad() {
        ...
        self.title = "DS Realtime Mỹ Phẩm"
        self.listDSRealtimeMyPham.removeAll()
        self.header = ["STT", "Shop", "Tổng DS", "SO", "DS TB/bill", "Trang điểm", "Chăm sóc da", "Chăm sóc cơ thể", "Chăm sóc tóc", "Khác"]
        self.getData()
    }
    ...
    func search(key:String){
        ...
        var list:[MyPhamRealtime] = []
        list.removeAll()
        if(key.count > 0) {
            for item in listDSRealtimeMyPham {
            ...
            self.generateData(listDSRealtimeMyPham)
            self.reportCollectionView.reloadData()
        }
    }
    ...
    func getData() {
        ...
        WaitingNetworkResponseAlert.PresentWaitingAlert(parentVC: self) {
            self.listDSRealtimeMyPham = mSMApiManager.ListDoanhSo_Realtime_ShopMyPham().Data ?? []
            if self.listDSRealtimeMyPham.count > 0 {
                WaitingNetworkResponseAlert.DismissWaitingAlert {
                    self.generateData(self.listDSRealtimeMyPham)
    ...
    func generateData(_ data:[MyPhamRealtime]) {
        cellData = [[String]]()
        for item in data {
            self.cellData.append([
                "\(item.STT!)",
                "\(item.TenShop!)",
                "\(Common.convertCurrencyDoubleV2(value: item.DS_Tong!))",
                "\(Common.convertCurrencyDoubleV2(value: item.SL_SO!))",
                "\(Common.convertCurrencyDoubleV2(value: item.TrungBinhBill!))",
                "\(Common.convertCurrencyDoubleV2(value: item.DS_TrangDiem!))",
                "\(Common.convertCurrencyDoubleV2(value: item.DS_CS_Da!))",
                "\(Common.convertCurrencyDoubleV2(value: item.DS_CS_CoThe!))",
                "\(Common.convertCurrencyDoubleV2(value: item.DS_CS_Toc!))",
                "\(Common.convertCurrencyDoubleV2(value: item.DS_Khac!))",
                ]);
        }
    }
    ...
    @objc func ExportFile() {
        let path = NSTemporaryDirectory() + "DSRealtimeShopMyPham.csv"
        
        //generate contents of file
        let data = self.listDSRealtimeMyPham
        var content = "STT\t Shop\t Tổng DS\t SO\t DS TB/bill\t Trang điểm\t Chăm sóc da\t Chăm sóc cơ thể\t Chăm sóc tóc\t Khác"
        for index in 0...data.count - 1 {
            let item = data[index]
            content += "\n\(item.STT!)\t \(item.TenShop!)\t \(item.DS_Tong!)\t \(item.SL_SO!)\t \(item.TrungBinhBill!)\t \(item.DS_TrangDiem!)\t \(item.DS_CS_Da!)\t \(item.DS_CS_CoThe!)\t \(item.DS_CS_Toc!)\t \(item.DS_Khac!)"
        }
        do {
            
            if #available(iOS 15.2, *) {
                let filName = "DSRealtimeShopMyPham.csv"
                ...
                let action = UIAlertAction(title: "OK", style: .default) { (_) in
                    ...
                    if( MFMailComposeViewController.canSendMail() ) {
                        ...
                        self.mailComposer.setSubject("mSM - DSRealtimeShopMyPham \(currentDateString)")
                        self.mailComposer.setMessageBody("DSRealtimeShopMyPham xuat tu ung dung mSM ngay \(currentDateString)", isHTML: false)
                        
                        if let fileData = try? Data(contentsOf: URL(fileURLWithPath: path)) {
                            print("File data loaded.")
                            self.mailComposer.addAttachmentData(fileData, mimeType: "text/csv", fileName: "DSRealtimeShopMyPham \(currentDateString).csv")
                        } else {
...
extension FBeautyReportViewController: UICollectionViewDelegate, UICollectionViewDataSource {
