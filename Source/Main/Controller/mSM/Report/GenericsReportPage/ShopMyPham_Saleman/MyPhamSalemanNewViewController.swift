//
//  MyPhamSalemanNewViewController.swift
//  fptshop
//
//  Created by Ngoc Bao on 7/5/21.
...
class MyPhamSalemanNewViewController: UIViewController, MFMailComposeViewControllerDelegate, UISearchBarDelegate {
    ...
    var listMPSaleman: [MyPhamSalemanNew] = []
    ...
    override func viewDidLoad() {
        ...
        self.title = "Báo cáo DS theo NV"
        self.listMPSaleman.removeAll()
        self.header = ["STT","Tên Shop", "Kênh", "Tên NV","Số SO","DS","TB bill","CS cơ thể","CS da","CS tóc", "Nước hoa", "PK","TPCN","Trang điểm"]
        self.getData()
    }
        ...
        if listMPSaleman.count > 0 {
            if(key.count > 0) {
                for item in listMPSaleman {
                    if ((item.TenShop.lowercased().range(of: key.lowercased())) != nil) || ((item.TenNV.lowercased().range(of: key.lowercased())) != nil) {
                        list.append(item)
                    }
                }
    ...
    func getData() {
        ...
        WaitingNetworkResponseAlert.PresentWaitingAlert(parentVC: self) {
            self.listMPSaleman = mSMApiManager.getMyphameSaleMan_new().Data ?? []
    ...
    func generateData(_ data:[MyPhamSalemanNew]) {
    ...
    @objc func ExportFile() {
        ...
        for index in 0...data.count - 1 {
            let item = data[index]
            content += "\n\(item.STT)\t \(item.TenShop)\t \(item.KenhBH)\t \(item.TenNV)\t \(item.SLSO)\t \(item.DS_MTD)\t \(item.DS_TBBill)\t \(item.CoThe)\t \(item.Da)\t \(item.Toc)\t \(item.NuocHoa)\t \(item.Khac)\t \(item.TPCN)\t \(item.TrangDiem)"
        }
        //write file
        do {
...
extension MyPhamSalemanNewViewController: UICollectionViewDelegate, UICollectionViewDataSource {
