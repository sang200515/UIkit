//
//  ShopMyPhamViewController.swift
//  fptshop
//
...
class ShopMyPhamViewController: UIViewController, MFMailComposeViewControllerDelegate, UISearchBarDelegate {
    ...
    override func viewDidLoad() {
        ...
        self.title = "BC DS Mỹ Phẩm Theo Shop"
        self.listShopMP.removeAll()
        self.header = ["Shop", "ASM", "DS MTD", "DS TB Ngày", "SO", "DS TB/Bill", "Target tháng trước", "DS Estimate", "% HT Target", "DS MTD tháng trước", "% Tăng giảm với DS tháng trước", "SL Cơ thể", "SL Da", "SL Nước hoa", "SL Tóc", "SL Trang điểm", "Khác", "Tổng SL"]
        self.getData()
    }
    ...
    func search(key:String){
        var list:[ShopMyPhamItem] = []
        list.removeAll()
        if listShopMP.count > 0 {
            if(key.count > 0) {
                for item in listShopMP {
    ...
    func getData() {
        ...
        WaitingNetworkResponseAlert.PresentWaitingAlert(parentVC: self) {
            self.listShopMP = mSMApiManager.ListDoanhSo_ShopMyPham_View_Final().Data ?? []
            if self.listShopMP.count > 0 {
                WaitingNetworkResponseAlert.DismissWaitingAlert {
                    self.generateData(self.listShopMP)
    ...
    func generateData(_ data:[ShopMyPhamItem]) {
        cellData = [[String]]()
        for item in data {
            self.cellData.append([
                "\(item.Shop)",
                "\(item.ASM)",
                "\(Common.convertCurrencyDouble(value: item.DS_MTD))",
                "\(Common.convertCurrencyDouble(value: item.DS_TB_Ngay))",
                "\(Common.convertCurrencyDouble(value: item.SLSO))",
                "\(Common.convertCurrencyDouble(value: item.DS_TBBill))",
                "\(Common.convertCurrencyDouble(value: item.Target_ThangTruoc))",
                "\(Common.convertCurrencyDouble(value: item.DS_Estimate))",
                "\(Common.convertCurrencyDouble(value: item.PT_HT_target))",
                "\(Common.convertCurrencyDouble(value: item.DS_MTD_ThangTruoc))",
                "\(Common.convertCurrencyDouble(value: item.PT_TangGiamVsDSThangTruoc))",
                "\(Common.convertCurrencyDouble(value: item.CoThe))",
                "\(Common.convertCurrencyDouble(value: item.Da))",
                "\(Common.convertCurrencyDouble(value: item.NuocHoa))",
                "\(Common.convertCurrencyDouble(value: item.Toc))",
                "\(Common.convertCurrencyDouble(value: item.TrangDiem))",
                "\(Common.convertCurrencyDouble(value: item.Khac))",
                "\(Common.convertCurrencyDouble(value: item.TongSL))"
            ]);
        }
    }
    ...
    @objc func ExportFile() {
        let path = NSTemporaryDirectory() + "DSMyPhamTheoShop.csv"
        
        //generate contents of file
        let data = self.listShopMP
        var content = self.header.joined(separator: "\t")
        for index in 0...data.count - 1 {
            let item = data[index]
            content += "\n\(item.Shop)\t \(item.ASM)\t \(item.DS_MTD)\t \(item.DS_TB_Ngay)\t \(item.SLSO)\t \(item.DS_TBBill)\t \(item.Target_ThangTruoc)\t \(item.DS_Estimate)\t \(item.PT_HT_target)\t \(item.DS_MTD_ThangTruoc)\t \(item.PT_TangGiamVsDSThangTruoc)\t \(item.CoThe)\t \(item.Da)\t \(item.NuocHoa)\t \(item.Toc)\t \(item.TrangDiem)\t \(item.Khac)\t \(item.TongSL)"
        }
        //write file
        do {
            if #available(iOS 15.2, *) {
                let filName = "DSMyPhamTheoShop.csv"
                ...
                let action = UIAlertAction(title: "OK", style: .default) { (_) in
                    ...
                    if( MFMailComposeViewController.canSendMail() ) {
                        ...
                        self.mailComposer.setSubject("mSM - DSMyPhamTheoShop \(currentDateString)")
                        self.mailComposer.setMessageBody("DSMyPhamTheoShop xuat tu ung dung mSM ngay \(currentDateString)", isHTML: false)
                        
                        if let fileData = try? Data(contentsOf: URL(fileURLWithPath: path)) {
                            print("File data loaded.")
                            self.mailComposer.addAttachmentData(fileData, mimeType: "text/csv", fileName: "DSMyPhamTheoShop \(currentDateString).csv")
                        } else {
...
extension ShopMyPhamViewController: UICollectionViewDelegate, UICollectionViewDataSource {
