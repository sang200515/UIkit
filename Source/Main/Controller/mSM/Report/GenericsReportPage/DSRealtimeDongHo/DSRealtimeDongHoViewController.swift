...
class DSRealtimeDongHoViewController: UIViewController, MFMailComposeViewControllerDelegate, UISearchBarDelegate {
    ...
    var listDSRealtimeDongHo: [DSRealTimeDongHoVung] = []
    ...
    override func viewDidLoad() {
        ...
        self.title = "BC DS Realtime Đồng Hồ"
        self.listDSRealtimeDongHo.removeAll()
        self.header = ["Vùng", "SL Shop", "SL Đồng Hồ", "DS Đồng Hồ", "TB Bill","SL TB Shop", "DS TB Shop"]
        self.getData()
    }
    ...
    func getData() {
        ...
        WaitingNetworkResponseAlert.PresentWaitingAlert(parentVC: self) {
            self.listDSRealtimeDongHo = mSMApiManager.FRT_usp_RP_PRO1_FR_FR124().Data ?? []
    ...
    func generateData(_ data:[DSRealTimeDongHoVung]) {
        cellData = [[String]]()
        for item in data {
            self.cellData.append([
                "\(item.Vung!)",
                "\(Common.convertCurrencyDouble(value: item.SL_Shop!))",
                "\(Common.convertCurrencyDouble(value: item.SL_DongHo!))",
                "\(Common.convertCurrencyDouble(value: item.DS_DongHo!))",
                "\(Common.convertCurrencyDouble(value: item.TB_Bill!))",
                "\(Common.convertCurrencyDouble(value: item.SL_TB_Shop!))",
                "\(Common.convertCurrencyDouble(value: item.SL_DS_Shop!))"
                ]);
        }
    }
    ...
    @objc func ExportFile() {
        ...
        var content = "Vùng\t Tên Shop\t Số lượng\t Doanh số\t TB Bill\t DS TB Shop"
        for index in 0...data.count - 1 {
            let item = data[index]
            content += "\n\(item.Vung!)\t \(item.SL_Shop!)\t \(item.SL_DongHo!)\t \(item.DS_DongHo!)\t \(item.TB_Bill!)\t \(item.SL_TB_Shop!)\t \(item.SL_DS_Shop!)"
        }
        //write file
        do {
            if #available(iOS 15.2, *) {
                ...
                let action = UIAlertAction(title: "OK", style: .default) { (_) in
                    ...
                    if( MFMailComposeViewController.canSendMail() ) {
                        ...
                        self.mailComposer.setSubject("mSM - DSRealtimeVungDSDongHo \(currentDateString)")
                        self.mailComposer.setMessageBody("DSRealtimeVungDongHo xuat tu ung dung mSM ngay \(currentDateString)", isHTML: false)
                        
                        if let fileData = try? Data(contentsOf: URL(fileURLWithPath: path)) {
                            print("File data loaded.")
                            self.mailComposer.addAttachmentData(fileData, mimeType: "text/csv", fileName: "DSRealtimeDongHoVung \(currentDateString).csv")
                        } else {
...
extension DSRealtimeDongHoViewController: UICollectionViewDelegate, UICollectionViewDataSource {
