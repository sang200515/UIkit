//
//  KhaiThacComboHomQuaViewController.swift
//  fptshop
//
...
class KhaiThacComboHomQuaViewController: UIViewController, MFMailComposeViewControllerDelegate, UISearchBarDelegate {
    ...
    var listKhaiThacCombo: [KhaiThacCombo] = []
    ...
    override func viewDidLoad() {
        ...
        self.navigationItem.title = "Khai thác combo hôm qua";
        ...
        self.listKhaiThacCombo.removeAll()
        self.header = ["STT", "Tên Shop", "Mã NV", "Tên NV", "SL Máy_3M", "Số CB Kèm", "Số Máy Chưa Kèm CB"]
        self.getKhaiThacCombo(username: self.username, token: self.token)
    }
    ...
    func search(key:String){
        ...
        var list:[KhaiThacCombo] = []
        list.removeAll()
        if(key.count > 0) {
            for item in listKhaiThacCombo {
                if ((item.TenShop?.lowercased().range(of: key.lowercased())) != nil || (item.TenNV?.lowercased().range(of: key.lowercased())) != nil || (item.MaNV?.lowercased().range(of: key.lowercased())) != nil) {
                    list.append(item)
                }
            }
            self.generateKhaiThacCombo(list)
            self.reportCollectionView.reloadData()
            
        } else {
            self.generateKhaiThacCombo(listKhaiThacCombo)
            self.reportCollectionView.reloadData()
        }
    }
    
    func getKhaiThacCombo(username: String, token: String) {
        cellData = [[String]]()
        WaitingNetworkResponseAlert.PresentWaitingAlert(parentVC: self) {
            
            let data = mSMApiManager.KhaiThacCombo_HomQua().Data ?? []
            if (!data.isEmpty){
                self.listKhaiThacCombo = data
                if self.listKhaiThacCombo.count > 0 {
                    self.generateKhaiThacCombo(self.listKhaiThacCombo)
    ...
    ///--GENERATE DATA-----------------------
    func generateKhaiThacCombo(_ data:[KhaiThacCombo]) {
        cellData = [[String]]()
        for item in data {
            self.cellData.append([
                ...
                "\(item.MaNV ?? "")",
                "\(item.TenNV ?? "")",
                "\(Common.convertCurrencyV2(value: item.SLMay3M ?? 0))",
                "\(Common.convertCurrencyV2(value: item.SoComboBanKemMay ?? 0))",
                "\(Common.convertCurrencyV2(value: item.SoMayChuaKemCombo ?? 0))"
                ]);
        }
    }
    ...
    @objc func ExportFile() {
        ...
        let path = NSTemporaryDirectory() + "ReportKhaiThacCombo.csv"
        
        //generate contents of file
        let data = self.listKhaiThacCombo
        var content = "STT\t Tên shop\t Mã NV\t Tên NV\t Số Máy_3M\t Số CB Kèm Máy\t Số Máy chưa kèm CB"
        for index in 0...data.count - 1 {
            let item = data[index]
            content += "\n\(item.STT ?? "")\t \(item.TenShop ?? "")\t \(item.MaNV ?? "")\t \(item.TenNV ?? "")\t \(item.SLMay3M ?? 0)\t \(item.SoComboBanKemMay ?? 0)\t \(item.SoMayChuaKemCombo ?? 0)"
        }
        //write file
        do {
            
            if #available(iOS 15.2, *) {
                let filName = "ReportKhaiThacCombo.csv"
                ...
                let action = UIAlertAction(title: "OK", style: .default) { (_) in
                    ...
                    if( MFMailComposeViewController.canSendMail() ) {
                        ...
                        self.mailComposer.setSubject("mSM - Xuat bao cao ReportKhaiThacCombo \(currentDateString)")
                        self.mailComposer.setMessageBody("ReportKhaiThacCombo xuat tu ung dung mSM ngay \(currentDateString)", isHTML: false)
                        
                        if let fileData = try? Data(contentsOf: URL(fileURLWithPath: path)) {
                            print("File data loaded.")
                            self.mailComposer.addAttachmentData(fileData, mimeType: "text/csv", fileName: "ReportKhaiThacCombo \(currentDateString).csv")
                        }
                        else {
...
extension KhaiThacComboHomQuaViewController: UICollectionViewDelegate, UICollectionViewDataSource {
