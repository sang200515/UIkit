//
//  BHMRRealtimeViewController.swift
//  fptshop
//
...
class BHMRRealtimeViewController: UIViewController, MFMailComposeViewControllerDelegate, UISearchBarDelegate {
    ...
    override func viewDidLoad() {
        ...
        case "BHMRVung":
            ...
            self.header = ["STT", "Vùng", "BHMR ĐTDĐ", "SL ĐTDĐ", "Tỷ lệ %", "BHMR khác", "SL khác", "Tỷ lệ %"]
            self.getDataVung()
            break
        ...
        case "BHMRASM" :
            ...
            self.listBHMR.removeAll()
            self.header = ["STT", "ASM", "Khu vực", "BHMR ĐTDĐ", "SL ĐTDĐ", "Tỷ lệ %", "BHMR khác", "SL khác", "Tỷ lệ %"]
            self.getDataASM()
            break
        ...
        case "BHMRShop" :
            ...
            self.listBHMR.removeAll()
            self.header = ["STT", "ASM", "Shop", "BHMR ĐTDĐ", "SL ĐTDĐ", "Tỷ lệ %", "BHMR khác", "SL khác", "Tỷ lệ %"]
            self.getDataShop()
            break
    ...
    func getDataVung() {
        ...
        WaitingNetworkResponseAlert.PresentWaitingAlert(parentVC: self) {
            self.listBHMR = mSMApiManager.ListSP_Realtime_SLSP_BHMR_Vung_View().Data ?? []
    ...
    func getDataASM() {
        cellData = [[String]]()
        WaitingNetworkResponseAlert.PresentWaitingAlert(parentVC: self) {
            self.listBHMR = mSMApiManager.ListSP_Realtime_SLSP_BHMR_ASM_View().Data ?? []
            if self.listBHMR.count > 0 {
                WaitingNetworkResponseAlert.DismissWaitingAlert {
    ...
    func getDataShop() {
        cellData = [[String]]()
        WaitingNetworkResponseAlert.PresentWaitingAlert(parentVC: self) {
            self.listBHMR = mSMApiManager.ListSP_Realtime_SLSP_BHMR_Shop_View().Data ?? []
            if self.listBHMR.count > 0 {
                WaitingNetworkResponseAlert.DismissWaitingAlert {
    ...
    @objc func ExportFile() {
        ...
        switch self.type {
        case "BHMRVung":
            let header = ["STT", "Vùng", "BHMR ĐTDĐ", "SL ĐTDĐ", "Tỷ lệ %", "BHMR khác", "SL khác", "Tỷ lệ %"]
            var content = header.joined(separator: "\t")
            for index in 0...data.count - 1 {
                let item = data[index]
                content += "\n\(item.STT)\t \(item.Vung)\t \(item.SLBHMR_DTDD)\t \(item.SLSP_DTDD)\t \(item.TyleKhaiThac_DTDD)\t \(item.SLBHMR_MTB_MTXT)\t \(item.SLSP_MTB_MTXT)\t \(item.TyleKhaiThac_MTB_MTXT)"
            }
        ...
        case "BHMRASM" :
            ...
            for index in 0...data.count - 1 {
                let item = data[index]
                content += "\n\(item.STT)\t \(item.ASM)\t \(item.Khuvuc)\t \(item.SLBHMR_DTDD)\t \(item.SLSP_DTDD)\t \(item.TyleKhaiThac_DTDD)\t \(item.SLBHMR_MTB_MTXT)\t \(item.SLSP_MTB_MTXT)\t \(item.TyleKhaiThac_MTB_MTXT)"
            }
            self.writeFile(path: path, content: content, reportName: reportName)
            break
            
        case "BHMRShop" :
            let header = ["STT", "ASM", "Shop", "BHMR ĐTDĐ", "SL ĐTDĐ", "Tỷ lệ %", "BHMR khác", "SL khác", "Tỷ lệ %"]
            var content = header.joined(separator: "\t")
            for index in 0...data.count - 1 {
                let item = data[index]
                content += "\n\(item.STT)\t \(item.ASM)\t \(item.TenShop)\t \(item.SLBHMR_DTDD)\t \(item.SLSP_DTDD)\t \(item.TyleKhaiThac_DTDD)\t \(item.SLBHMR_MTB_MTXT)\t \(item.SLSP_MTB_MTXT)\t \(item.TyleKhaiThac_MTB_MTXT)"
            }
            self.writeFile(path: path, content: content, reportName: reportName)
            break
        default:
...
extension BHMRRealtimeViewController: UICollectionViewDelegate, UICollectionViewDataSource {
