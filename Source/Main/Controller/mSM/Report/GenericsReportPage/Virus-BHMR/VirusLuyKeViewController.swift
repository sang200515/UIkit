//
//  VirusLuyKeViewController.swift
//  fptshop
//
...
class VirusLuyKeViewController: UIViewController, MFMailComposeViewControllerDelegate, UISearchBarDelegate {
    ...
    override func viewDidLoad() {
        ...
        switch self.type {
        case "VirusVung":
            self.title = "BC Luỹ Kế Tháng PM Diệt Virus Eset Vùng"
        ...
        case "VirusASM" :
            self.title = "BC Luỹ Kế Tháng PM Diệt Virus Eset Khu Vực"
            self.listVirus.removeAll()
        ...
        case "VirusShop" :
            self.title = "BC Luỹ Kế Tháng PM Diệt Virus Eset Shop"
            self.listVirus.removeAll()
    ...
    func getDataVung() {
        ...
        WaitingNetworkResponseAlert.PresentWaitingAlert(parentVC: self) {
            self.listVirus = mSMApiManager.ListSP_LuyKe_SLSP_Virus_Vung_View().Data ?? []
    ...
    func getDataASM() {
        cellData = [[String]]()
        WaitingNetworkResponseAlert.PresentWaitingAlert(parentVC: self) {
            self.listVirus = mSMApiManager.ListSP_LuyKe_SLSP_Virus_ASM_View().Data ?? []
            if self.listVirus.count > 0 {
                WaitingNetworkResponseAlert.DismissWaitingAlert {
    ...
    func getDataShop() {
        cellData = [[String]]()
        WaitingNetworkResponseAlert.PresentWaitingAlert(parentVC: self) {
            self.listVirus = mSMApiManager.ListSP_LuyKe_SLSP_Virus_Shop_View().Data ?? []
            if self.listVirus.count > 0 {
                WaitingNetworkResponseAlert.DismissWaitingAlert {
    ...
    func exportVirusVung() {
        let path = NSTemporaryDirectory() + "ReportLuyKeVirusVung.csv"
        ...
        //write file
        self.writeFile(path: path, content: content, reportName: "ReportLuyKeVirusVung")
    }

    func exportVirusASM() {
        let path = NSTemporaryDirectory() + "ReportLuyKeVirusASM.csv"
        ...
        //write file
        self.writeFile(path: path, content: content, reportName: "ReportLuyKeVirusASM")
    }
    
    func exportVirusShop() {
        let path = NSTemporaryDirectory() + "ReportLuyKeVirusShop.csv"
        ...
        //write file
        self.writeFile(path: path, content: content, reportName: "ReportLuyKeVirusShop")
    }
...
extension VirusLuyKeViewController: UICollectionViewDelegate, UICollectionViewDataSource {
