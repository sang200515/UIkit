//
//  TyLePKRealtimeViewController.swift
//  fptshop
//
...
class TyLePKRealtimeViewController: UIViewController, MFMailComposeViewControllerDelegate, UISearchBarDelegate {
    ...
    var listSP: [TyLePhuKien_Realtime] = []
    var type = ""
    ...
    override func viewDidLoad() {
        ...
        switch self.type {
        case "TLPK_Vung":
            self.title = "Báo cáo realtime tỷ trọng PK Vùng"
            self.listSP.removeAll()
            self.header = ["Vùng", "Tổng", "Phụ Kiện ALL", "TT PK ALL", "CB 10%", "CB 15%", "CB 20%"]
            self.getDataVung()
            break
        case "TLPK_KV":
            self.title = "Báo cáo realtime tỷ trọng PK khu vực"
            self.listSP.removeAll()
            self.header = ["Tên KV", "Tên ASM", "Tổng", "Phụ Kiện ALL", "TT PK ALL", "CB 10%", "CB 15%", "CB 20%"]
            self.getDataKV()
            break
        default:
            break
    ...
    func getDataVung() {
        ...
        WaitingNetworkResponseAlert.PresentWaitingAlert(parentVC: self) {
            self.listSP = mSMApiManager.DoanhSo_TyLePhuKien_Realtime_Vung_View().Data ?? []
    ...
    func getDataKV() {
        cellData = [[String]]()
        WaitingNetworkResponseAlert.PresentWaitingAlert(parentVC: self) {
            self.listSP = mSMApiManager.DoanhSo_TyLePhuKien_Realtime_KhuVuc_View().Data ?? []
            if self.listSP.count > 0 {
                WaitingNetworkResponseAlert.DismissWaitingAlert {
    ...
    func generateDataVung(_ data:[TyLePhuKien_Realtime]) {
        cellData = [[String]]()
        for item in data {
            self.cellData.append([
                ...
                "\(item.TyTrong_Combo_10)",
                "\(item.TyTrong_Combo_15)",
                "\(item.TyTrong_Combo_20)"
            ]);
        }
    }
    func generateDataKV(_ data:[TyLePhuKien_Realtime]) {
        cellData = [[String]]()
        for item in data {
            self.cellData.append([
                "\(item.TenKV)",
    ...
    @objc func ExportFile() {
        ...
        switch self.type {
        case "TLPK_Vung":
            let header =  ["Vùng", "Tổng", "Phụ Kiện ALL", "TT PK ALL", "CB 10%", "CB 15%", "CB 20%"]
            var content = header.joined(separator: "\t")
            for index in 0...data.count - 1 {
                let item = data[index]
                content += "\n\(item.Vung)\t \(item.DoanhSo_All)\t \(item.DoanhSo_PK)\t \(item.TyLe_PhuKien_ALL)\t \(item.TyTrong_Combo_10)\t \(item.TyTrong_Combo_15)\t \(item.TyTrong_Combo_20)"
            }
            self.writeFile(path: path, content: content, reportName: "BCRealtimeTiTrongPhuKien_Vung")
            break
        case "TLPK_KV":
            let header =  ["Tên KV", "Tên ASM", "Tổng", "Phụ Kiện ALL", "TT PK ALL", "CB 10%", "CB 15%", "CB 20%"]
            var content = header.joined(separator: "\t")
            for index in 0...data.count - 1 {
                let item = data[index]
                content += "\n\(item.TenKV)\t \(item.TenASM)\t \(item.DoanhSo_All)\t \(item.DoanhSo_PK)\t \(item.TyLe_PhuKien_ALL)\t \(item.TyTrong_Combo_10)\t \(item.TyTrong_Combo_15)\t \(item.TyTrong_Combo_20)"
            }
            self.writeFile(path: path, content: content, reportName: "BCRealtimeTiTrongPhuKien_KV")
            break
        default:
            break
        }
...
extension TyLePKRealtimeViewController: UICollectionViewDelegate, UICollectionViewDataSource {
