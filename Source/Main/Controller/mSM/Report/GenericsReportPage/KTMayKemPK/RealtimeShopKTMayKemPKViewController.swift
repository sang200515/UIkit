//
//  RealtimeShopKTMayKemPKViewController.swift
//  fptshop
//
...
class RealtimeShopKTMayKemPKViewController: UIViewController, MFMailComposeViewControllerDelegate, UISearchBarDelegate {
    ...
    override func viewDidLoad() {
        ...
        self.title = "BC Realtime khai thác máy kèm PK"
        self.listKTMayKemPK.removeAll()
        self.header = ["Vùng", "ASM", "Tên Shop", "SL Máy", "SL SDP", "% KT", "SL MDMH", "% KT", "SL Ốp lưng", "% KT"]
        self.getData()
    }
    ...
    func search(key:String){
        var list:[KTMayKemPK] = []
        list.removeAll()
        if listKTMayKemPK.count > 0 {
            if(key.count > 0) {
                for item in listKTMayKemPK {
                    if ((item.Vung.lowercased().range(of: key.lowercased())) != nil) || ((item.ASM.lowercased().range(of: key.lowercased())) != nil) || ((item.TenShop.lowercased().range(of: key.lowercased())) != nil) {
                        list.append(item)
                    }
                }
                ...
                self.generateData(listKTMayKemPK)
                self.reportCollectionView.reloadData()
            }
        } else {
    ...
    func getData() {
        ...
        WaitingNetworkResponseAlert.PresentWaitingAlert(parentVC: self) {
            self.listKTMayKemPK = mSMApiManager.msm_RealtimeShopKhaiThacMayKemPK().Data ?? []
            if self.listKTMayKemPK.count > 0 {
                WaitingNetworkResponseAlert.DismissWaitingAlert {
                    self.generateData(self.listKTMayKemPK)
    ...
    func generateData(_ data:[KTMayKemPK]) {
        cellData = [[String]]()
        for item in data {
            self.cellData.append([
                ...
                "\(item.ASM)",
                "\(item.TenShop)",
                "\(Common.convertCurrencyDouble(value: item.SLMay))",
                "\(Common.convertCurrencyDouble(value: item.SLSDP))",
                "\(Common.convertCurrencyDouble(value: item.TyleKTSDP))%",
                "\(Common.convertCurrencyDouble(value: item.SLMDMH))",
                "\(Common.convertCurrencyDouble(value: item.TyleKTMDMH))%",
                "\(Common.convertCurrencyDouble(value: item.SLOplung))",
                "\(Common.convertCurrencyDouble(value: item.TyleKTOplung))%"
            ]);
        }
    }
    ...
    @objc func ExportFile() {
        let path = NSTemporaryDirectory() + "BCRealtimeKhaiThacMayKemPK.csv"
        
        //generate contents of file
        let data = self.listKTMayKemPK
        var content = self.header.joined(separator: "\t")
        for index in 0...data.count - 1 {
            let item = data[index]
            content += "\n\(item.Vung)\t \(item.ASM)\t \(item.TenShop)\t \(item.SLMay)\t \(item.SLSDP)\t \(item.TyleKTSDP)\t \(item.SLMDMH)\t \(item.TyleKTMDMH)\t \(item.SLOplung)\t \(item.TyleKTOplung)"
        }
        //write file
        do {
            
            if #available(iOS 15.2, *) {
                let filName = "BCRealtimeKhaiThacMayKemPK.csv"
                ...
                let action = UIAlertAction(title: "OK", style: .default) { (_) in
                    ...
                    if( MFMailComposeViewController.canSendMail() ) {
                        ...
                        self.mailComposer.setSubject("mSM - BCRealtimeKhaiThacMayKemPK \(currentDateString)")
                        self.mailComposer.setMessageBody("BCRealtimeKhaiThacMayKemPK xuat tu ung dung mSM ngay \(currentDateString)", isHTML: false)
                        
                        if let fileData = try? Data(contentsOf: URL(fileURLWithPath: path)) {
                            print("File data loaded.")
                            self.mailComposer.addAttachmentData(fileData, mimeType: "text/csv", fileName: "BCKhaiThacMayKemPK \(currentDateString).csv")
                        } else {
...
extension RealtimeShopKTMayKemPKViewController: UICollectionViewDelegate, UICollectionViewDataSource {
