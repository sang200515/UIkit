...
class BaoHiemXeViewController: UIViewController, MFMailComposeViewControllerDelegate, UISearchBarDelegate {
    ...
    func setupHeader() {
        if type == .xeVung {
            self.title = "Báo cáo BH theo vùng"
            self.header = ["STT", "Vùng","Realtime","Lũy kế (D-1)","Thu hộ"," Tỉ lệ "]
        }  else if type == .xeKhuVuc {
            self.title = "Báo cáo BH theo khu vực"
            self.header = ["STT","ASM", "Khu vực","Realtime","Lũy kế (D-1)","Thu hộ"," Tỉ lệ "]
        }  else if type == .xeShop {
            self.title = "Báo cáo BH theo Shop"
            self.header = ["STT","ASM", "Shop","Realtime","Lũy kế (D-1)","Thu hộ"," Tỉ lệ "]
        } else {
            self.header = []
        }
    }
        ...
        if listReport.count > 0 {
            if(key.count > 0) {
                for item in listReport {
                    if ((item.vung.lowercased().range(of: key.lowercased())) != nil) || ((item.khuvuc.lowercased().range(of: key.lowercased())) != nil) || ((item.aSM.lowercased().range(of: key.lowercased())) != nil) {
                        list.append(item)
                    }
                }
    ...
    func getData() {
        self.listReport = []
        cellData = [[String]]()
        Provider.shared.baoHanhMSMService.getBaoHiemXe(type: type,success: { [weak self] result in
    ...
    func generateData(_ data:[MotorItem]) {
        cellData = [[String]]()
        for item in data {
            switch type {
            case .xeShop:
    ...
    @objc func ExportFile() {
        var name = ""
        let path = NSTemporaryDirectory() + "BaoHiemxe.csv"
        ...
        for index in 0...data.count - 1 {
            ...
            switch type {
            case .xeShop:
                content += "\n\(item.sTT)\t\(item.aSM)\t\(item.shop)\t\(item.realtime)\t\(item.luyKe)\t\(item.thuho_BaoHanh)\t\(item.tile)"
                name = "BaoHiemXeShop"
            case .xeVung:
                content += "\n\(item.sTT)\t\(item.vung)\t\(item.realtime)\t\(item.luyKe)\t\(item.thuho_BaoHanh)\t\(item.tile)"
                name = "BaoHiemXeVung"
            case .xeKhuVuc:
                content += "\n\(item.sTT)\t\(item.aSM)\t\(item.khuvuc)\t\(item.realtime)\t\(item.luyKe)\t\(item.thuho_BaoHanh)\t\(item.tile)"
                name = "BaoHiemXeKhuVuc"
            default:
                break
            }
...
extension BaoHiemXeViewController: UICollectionViewDelegate, UICollectionViewDataSource {
