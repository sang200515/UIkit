//
//  DetailBillInfoViewController.swift
//  fptshop
//
...
class DetailBillInfoViewController: UIViewController {
...
//        tfPackageService.text = selectedDichVu?.Name ?? ""
...
//        let tapShowNVC = UITapGestureRecognizer(target: self, action: #selector(showListNhaVanChuyen))
//        tfPackageDistribute.isUserInteractionEnabled = true
//        tfPackageDistribute.addGestureRecognizer(tapShowNVC)
...
//        tfPackageDistribute.rightView = arrowImgView1
        ...
        tfPackagePaymentType.rightView = arrowImgView3
...
//            self.loadDiaChiNhan(maPB: self.receiverObj?.OrganizationHierachyCode ?? "")
...
//
//        //            self.loadNhaVanChuyen()
////        if let nhaVanChuyen = self.tfPackageDistribute.text, nhaVanChuyen.isEmpty {
//
//        WaitingNetworkResponseAlert.PresentWaitingAlert(parentVC: self) {
...
//                    self.tfPackageDistribute.text = self.listDistribute[0].TenNVC ?? ""
//                    let cuocPhi = Int(self.listDistribute[0].CuocPhiMin ?? "")
...
//                                p_MaDichVuVanChuyen: self.selectedDistribute?.MaDichVu ?? "",
//                                p_MaHinhThucThanhToan: "\(self.selectedHTTT?.Code ?? 0)",
//                                p_GhiChu: self.tfPackageNote.text ?? "",
//                                p_CuocPhiMin: "\(self.selectedDistribute?.CuocPhiMin ?? "")").Data ?? []
//
//                            WaitingNetworkResponseAlert.DismissWaitingAlert {
...
//        newViewController.senderAddess = self.lblSenderAddressText.text ?? ""
...
extension DetailBillInfoViewController: UITextFieldDelegate {
