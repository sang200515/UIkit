...
class PromotionMiraeViewController: UIViewController,UITableViewDelegate, UITableViewDataSource, ChangePromotionViewControllerDelegate {
    ...
    override func viewDidLoad() {
        ...
        btBackIcon.addTarget(self, action: #selector(PromotionMiraeViewController.actionBack), for: UIControl.Event.touchUpInside)
        ...
        self.promotions = Cache.promotionsMirae
        self.group = Cache.groupMirae
        ...
        Cache.resultPromostionsMirae.removeAll()
        Cache.resultPromostionsMirae.append(contentsOf: list)
        self.setupUI()
    }
        ...
        if groupChoose.count > 0 || notPromotionValue{
            ...
            for item in self.result{
                if(item.group == groupChoose){
                    ...
                    for itm in item.promotions!{
                        Cache.itemsPromotionMirae.append(itm.promotionsMain)
                    }
                    break
                }
            }
            ...
            if ShinhanData.IS_RUNNING {
                ...
                if self.count == 0 {
                    ...
                    vc.presenter?.cachePromotions = Cache.itemsPromotionMirae
                    self.navigationController?.pushViewController(vc, animated: true)
                }else {
                    let vc = UpdateGoiVayMiraeRouter().configureVIPERUpdateGoiVayMirae()
                    vc.presenter?.count = 1
                    self.navigationController?.pushViewController(vc, animated: true)
                }
            }
        }else{
    ...
    func updateTotal(discountPrice: Float){
        var sum: Float = 0
        if ShinhanData.IS_RUNNING && ShinhanData.detailorDerHistory != nil {
...
extension PromotionMiraeViewController : FZAccordionTableViewDelegate {
