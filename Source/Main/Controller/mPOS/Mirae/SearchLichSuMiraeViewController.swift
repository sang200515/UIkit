...
import PopupDialog
class SearchLichSuMiraeViewController: UIViewController,UITableViewDataSource, UITableViewDelegate {
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        self.title = PARTNERID == "FPT" ? "Lịch sử mua hàng" : "Lịch sử thuê máy"
        ...
        // Setup the Scope Bar
        searchController.searchBar.scopeButtonTitles = ["CMND", "SĐT", "Số HĐ" ,]
        searchController.searchBar.delegate = self
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        MPOSAPIManager.mpos_FRT_SP_mirae_history_order_byuser(handler: { [weak self](results, err) in
    ...
    func setupUI(list: [HistoryOrderByUser]){
        ...
        tableView.register(SearchItemSOHistoryMiraeTableViewCell.self, forCellReuseIdentifier: "SearchItemSOHistoryMiraeTableViewCell")
    ...
    {
        let cell = SearchItemSOHistoryMiraeTableViewCell(style: UITableViewCell.CellStyle.default, reuseIdentifier: "SearchItemSOHistoryMiraeTableViewCell")
        let item:HistoryOrderByUser = items[indexPath.row]
        cell.setup(so: item)
    ...
    func filterContentForSearchText(_ searchText: String, scope: String = "Tên") {
        if(searchText.count > 0){
            ...
            if (scope == "CMND"){
                ...
                //                self.items.removeAll()
                //
                //                self.items = self.listTemp.filter { "\($0.processId_Mirae)".contains(searchText) }
                //                tableView.reloadData()
                self.actionSearch(Keyword:searchText,Type:"2")
            }
    ...
    func actionSearch(Keyword:String,Type:String){
        MPOSAPIManager.mpos_FRT_SP_Mirae_history_order_byKeyword(Keyword:Keyword,Type:Type,handler: {[weak self] (results, err) in
            guard let self = self else {return}
            if(results.count > 0){
...
extension SearchLichSuMiraeViewController: UISearchBarDelegate {
...
extension SearchLichSuMiraeViewController: UISearchResultsUpdating {
...
class SearchItemSOHistoryMiraeTableViewCell: UITableViewCell {
    ...
    func setup(so:HistoryOrderByUser){
        ...
        ngay.frame = CGRect(x: donhang.frame.origin.x + donhang.frame.size.width , y: Common.Size(s:10) ,width: UIScreen.main.bounds.size.width - Common.Size(s:20) ,height: Common.Size(s:16))
        ngay.text = "\(so.Ngay)"
        
        tenKH.frame = CGRect(x: Common.Size(s:10),y: donhang.frame.origin.y + donhang.frame.size.height + Common.Size(s:10) ,width: UIScreen.main.bounds.size.width - Common.Size(s:20) ,height: Common.Size(s:16))
...
//
//        trangthaiCallLog.frame = CGRect(x: Common.Size(s:10),y: callLogImage.frame.origin.y + callLogImage.frame.size.height + Common.Size(s:10) ,width: UIScreen.main.bounds.size.width - Common.Size(s:20) ,height: Common.Size(s:16))
...
//             trangthaiCallLog.sizeToFit()
        ...
        soHD.frame = CGRect(x: Common.Size(s:10),y: trangthaiDH.frame.origin.y + trangthaiDH.frame.size.height + Common.Size(s:10) ,width: UIScreen.main.bounds.size.width - Common.Size(s:20) ,height: Common.Size(s:16))
