...
import PopupDialog
class LichSuMuaHangMiraeViewController: UIViewController,UITableViewDataSource, UITableViewDelegate {
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        MPOSAPIManager.mpos_FRT_SP_mirae_history_order_byuser(handler: { (results, err) in
            if(results.count > 0){
    ...
    func setupUI(list: [HistoryOrderByUser]){
        ...
        tableView.register(ItemSOHistoryMiraeTableViewCell.self, forCellReuseIdentifier: "ItemSOHistoryMiraeTableViewCell")
    ...
    {
        let cell = ItemSOHistoryMiraeTableViewCell(style: UITableViewCell.CellStyle.default, reuseIdentifier: "ItemSOHistoryMiraeTableViewCell")
        let item:HistoryOrderByUser = items[indexPath.row]
        cell.setup(so: item)
...
class ItemSOHistoryMiraeTableViewCell: UITableViewCell {
...
//        trangthaiCallLog.sizeToFit()
        ...
        self.estimateCellHeight = tinhtrang.frame.origin.y + tinhtrang.frame.height + Common.Size(s: 15)
