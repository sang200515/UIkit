//
//  ThongTinDonHangTableViewCell.swift
//  MireaKetNoiHeThong
//
...
class ThongTinDonHangMiraeTableViewCell : UITableViewCell {
    ...
    private func bindData(){
        self.tenSPLabel.text = "\(self.model?.product.sku ?? "") - \(self.model?.product.name ?? "")"
		if model?.product.hotSticker ?? false {
			let imageAttachment = NSTextAttachment()
			imageAttachment.image = UIImage(named: "ic_hot3")
			let imageString = NSAttributedString(attachment: imageAttachment)
			imageAttachment.bounds = CGRect(x: -10, y: 0, width: 65, height: 23)
			let textString = NSAttributedString(string: " \(self.model?.product.sku ?? "") - \(self.model?.product.name ?? "")")
        ...
        self.giaSPLabel.text = Common.convertCurrencyFloat(value: self.model?.price ?? 0)
