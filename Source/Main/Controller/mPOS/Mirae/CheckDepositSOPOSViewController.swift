...
import UIKit
import PopupDialog
class CheckDepositSOPOSViewController: UIViewController,UITextFieldDelegate {
    ...
    override func viewDidLoad() {
        ...
        btPlusIcon.addTarget(self, action: #selector(CheckDepositSOPOSViewController.backButton), for: UIControl.Event.touchUpInside)
        ...
        lblTitle.text = "Nhập số đơn hàng đặt cọc / số điện thoại"
        ...
        lblSOPOS = UILabel(frame: CGRect(x: Common.Size(s:15), y: label3.frame.size.height + label3.frame.origin.y + Common.Size(s:10), width: scrollView.frame.size.width - Common.Size(s:30), height: Common.Size(s:14)))
        lblSOPOS.textAlignment = .left
        lblSOPOS.textColor = UIColor.black
        lblSOPOS.font = UIFont.systemFont(ofSize: Common.Size(s:14))
        lblSOPOS.text = "Số POS"
        viewHeader.addSubview(lblSOPOS)
        

        
        lblSOMPOS = UILabel(frame: CGRect(x: Common.Size(s:15), y:lblSOPOS.frame.size.height + lblSOPOS.frame.origin.y + Common.Size(s:10), width: scrollView.frame.size.width - Common.Size(s:30), height: Common.Size(s:14)))
        lblSOMPOS.textAlignment = .left
        lblSOMPOS.textColor = UIColor.black
        lblSOMPOS.font = UIFont.systemFont(ofSize: Common.Size(s:14))
        lblSOMPOS.text = "Số MPOS"
        viewHeader.addSubview(lblSOMPOS)
        
   
        
        lblEcom = UILabel(frame: CGRect(x: Common.Size(s:15), y:lblSOMPOS.frame.size.height + lblSOMPOS.frame.origin.y + Common.Size(s:10), width: scrollView.frame.size.width - Common.Size(s:30), height: Common.Size(s:14)))
        ...
        lblGiaSP = UILabel(frame: CGRect(x: Common.Size(s:15), y: lblTenSP.frame.origin.y + lblTenSP.frame.size.height + Common.Size(s:10), width: scrollView.frame.size.width - Common.Size(s:30), height: Common.Size(s:14)))
        lblGiaSP.textAlignment = .left
        lblGiaSP.textColor = UIColor.red
        lblGiaSP.font = UIFont.systemFont(ofSize: Common.Size(s:14))
        viewHeader.addSubview(lblGiaSP)
        
        lblSoTienCoc = UILabel(frame: CGRect(x: Common.Size(s:15), y: lblGiaSP.frame.origin.y + lblGiaSP.frame.size.height + Common.Size(s:10), width: scrollView.frame.size.width - Common.Size(s:30), height: Common.Size(s:14)))
...
//        dismiss(animated: false, completion: nil)
    ...
    @objc func actionSkip(sender: UIBarButtonItem){
        ...
        Cache.infoCustomerMirae!.pre_docentry = ""
    ...
    func actionnCheck(){
        ...
        MPOSAPIManager.mpos_FRT_SP_mirae_load_by_SOPOS(SOPOS: "\(self.tfInput.text!)",docentry_mpos_HD:"\(Cache.infoCustomerMirae!.IDMPOS)") { (status,header,detail,err) in
                    ...
                    if(status.count > 0){
                        if(status[0].p_status == 1){
                            if(header.count > 0){
                                ...
                                self.lblSOPOS.text = "Số POS: \(header[0].SOPOS)"
                                self.lblSOMPOS.text = "Số MPOS: \(header[0].SOMPOS)"
                                self.lblEcom.text = "Số Ecom: \(header[0].U_NumECom)"
                                self.lblSoTienCoc.text = "Số tiền cọc: \(Common.convertCurrencyFloat(number: header[0].SoTienCoc)) VNĐ"
                                Cache.infoCustomerMirae!.pre_docentry = "\(header[0].SOMPOS)"
                                Cache.soTienCocMirae = header[0].SoTienCoc
                            ...
                            if(detail.count > 0){
                                ...
                                self.scrollView.contentSize = CGSize(width: self.view.frame.size.width, height: self.viewHeader.frame.origin.y + self.viewHeader.frame.size.height  + Common.Size(s:100))
