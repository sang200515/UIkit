//
//  DidUpdateMayCuViewController.swift
//  fptshop
//
...
class DidUpdateMayCuViewController: UIViewController {
    ...
    func loadData() {
        MPOSAPIManager.Maycu_Ecom_GetListProduct(status: "2") { (rs, err) in
            if err.count <= 0 {
                if rs.count > 0 {
                    ...
                    let popup = PopupDialog(title: "Thông báo", message: "Không có danh sách cập nhật ảnh SP cũ đã cập nhật!", buttonAlignment: .horizontal, transitionStyle: .zoomIn, tapGestureDismissal: false, panGestureDismissal: false, hideStatusBar: false) {
                        print("Completed")
                    }
...
extension DidUpdateMayCuViewController: UITableViewDelegate, UITableViewDataSource {
