//
//  BackToSchoolNewVC.swift
//  fptshop
//
//  Created by Ngoc Bao on 6/14/21.
//  Copyright © 2021 FPT DIGITAL RETAIL JOINT STOCK COMPANY. All rights reserved.
//

import UIKit

class BackToSchoolNewVC: BaseController {
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        showDatePicker()
        self.title = isNewStudent ? "TÂN SINH VIÊN" : "CHƯƠNG TRÌNH DÀNH CHO HS/SV"
                            ...
                            if success == "1" {
                                if result != nil {
                                    if result?.Result == 1 || result?.Result == 2 {
                                        ...
                                        strongSelf.averageScore.text = info.DiemTrungBinh
    ...
    @IBAction func onConfirm(_ sender: Any) {
        ...
        if isNewStudent {
            ...
            WaitingNetworkResponseAlert.PresentWaitingAlert(parentVC: self) {
                MPOSAPIManager.backToSchool_InsertDataStudentFPT(name: self.nameTxt.text ?? "", identity: self.cmndNumberTxt.text ?? "", phoneNumber: self.sdtTxt.text ?? "", user: user, type: 3,birthday: self.birthdayTxt.text ?? "",shipType: newtype) {[weak self] (success, mErr, result, err) in
                        guard let strongSelf = self else {return}
...
extension BackToSchoolNewVC: UINavigationControllerDelegate, UIImagePickerControllerDelegate{
