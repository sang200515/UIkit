//
//  DetailInfoPointEachSubjectCell.swift
//  fptshop
//
...
class DetailInfoPointEachSubjectCell: BaseTableCell {
    ...
    func setupViews(_ item: DetailInfoStudentFPTItem.Data) {
        ...
        let pointView = UIView(frame: CGRect(x: 0, y: 0, width: 200, height: 150))
        contentView.addSubview(pointView)
        pointView.myCustomAnchor(top: lbSBD.bottomAnchor, leading: lbSBD.leadingAnchor, trailing: self.contentView.trailingAnchor, bottom: nil, centerX: nil, centerY: nil, width: nil, height: nil, topConstant: -4, leadingConstant: 0, trailingConstant: 8, bottomConstant: 0
            , centerXConstant: 0, centerYConstant: 0, widthConstant: 0, heightConstant: 0)
        ...
        if item.diemTungMon?.count ?? 0 > 0 {
            for i in 1...(item.diemTungMon?.count ?? 0) {
        ...
        let pointViewHeight:CGFloat = ((CGFloat((item.diemTungMon?.count ?? 0)/2) + 1) * Common.Size(s:20)) + Common.Size(s:20)
