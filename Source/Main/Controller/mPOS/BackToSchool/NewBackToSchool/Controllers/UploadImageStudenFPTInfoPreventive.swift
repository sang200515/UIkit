//
//  UploadImageStudenFPTInfoPreventive.swift
//  fptshop
//
...
class UploadImageStudenFPTInfoPreventive: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        //img giay bao thi
        lbImgGiayBaoThi = UILabel(frame: CGRect(x: Common.Size(s:15), y: cmndSauView.frame.origin.y + cmndSauView.frame.height + Common.Size(s:15), width: lbImgCmndTruoc.frame.width - Common.Size(s: 15), height: Common.Size(s:35)))
        lbImgGiayBaoThi.numberOfLines = 0
        lbImgGiayBaoThi.lineBreakMode = .byClipping
        lbImgGiayBaoThi.text = "Hình ảnh Thẻ SV/ Thẻ HS/ Giấy khai sinh/ Giấy tờ liên quan"
        lbImgGiayBaoThi.textColor = UIColor.black
        lbImgGiayBaoThi.font = UIFont.systemFont(ofSize: 13)
    ...
    @objc func uploadImg() {
        ...
        if isCheck == false {
            ...
            WaitingNetworkResponseAlert.PresentWaitingAlert(parentVC: self) {
                MPOSAPIManager.BackToSchool_UpdateHinhAnh(ID_BackToSchool: self.iD_BackToSchool, SBD: "0", Url_CMND_MT: self.urlCMNDTruoc, Url_CMND_MS: self.urlCMNDSau, Url_GiayBaoDuThi: self.urlGiayBaoThi, handler: { [weak self] (success, errorMsg, mData, err) in
                    ...
                    WaitingNetworkResponseAlert.DismissWaitingAlert {
                        if success == "1" {
                            ...
                            strongSelf.showAlert(title: "Thông báo", message: errorMsg)
                        }
                    }
                })
            }
                ...
                WaitingNetworkResponseAlert.DismissWaitingAlert {
                    if success == "1" {
                        ...
                        let action = UIAlertAction(title: "OK", style: .default, handler: { (_) in
                            ...
                            self.urlGiayBaoThi = ""
                            self.imgGiayBaoThi.image = UIImage.init(named: "img_entrance_examination_update")
                        })
                        alertVC.addAction(action)
                        self.present(alertVC, animated: true, completion: nil)
    ...
    func getNumberIdentity(_ identity: String) {
        self.numberIdentity = identity
    }
}

extension UploadImageStudenFPTInfoPreventive: UINavigationControllerDelegate, UIImagePickerControllerDelegate {
