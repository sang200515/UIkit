//
//  BackToSchoolPolytechnic.swift
//  fptshop
//
//  Created by Sang Trương on 09/03/2023.
//  Copyright © 2023 FPT DIGITAL RETAIL JOINT STOCK COMPANY. All rights reserved.
//

import UIKit

class BackToSchoolPolytechnicVC: BaseController {
	...
	@IBAction func onConfirm(_ sender: Any) {
		...
		if isNewStudent {
			...
			WaitingNetworkResponseAlert.PresentWaitingAlert(parentVC: self) {
				MPOSAPIManager.backToSchool_InsertDataStudentFPT(name: self.nameTxt.text ?? "", identity: self.cmndNumberTxt.text ?? "", phoneNumber: self.sdtTxt.text ?? "", user: user, type: 5,birthday: self.birthdayTxt.text ?? "",shipType: newtype) {[weak self] (success, mErr, result, err) in
					guard let strongSelf = self else {return}
...
extension BackToSchoolPolytechnicVC: UINavigationControllerDelegate, UIImagePickerControllerDelegate{
