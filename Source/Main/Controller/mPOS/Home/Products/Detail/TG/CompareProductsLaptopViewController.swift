//
//  CompareProductsLaptopViewController.swift
//  fptshop
//
...
import PopupDialog
class CompareProductsLaptopViewController: UIViewController{
    ...
    override func viewDidLoad() {
        ...
        btBackIcon.addTarget(self, action: #selector(CompareProductsLaptopViewController.actionBack), for: UIControl.Event.touchUpInside)
    ...
    func initUI(compareProduct:ProductBySku,listSPLenDoi:[LenDoiSP]){
        ...
        lbNameProduct2.numberOfLines = 10
        ...
        var dictionaryProduct =  [String:String]()
        if(product.atrribute.count > 0){
            for item in product.atrribute {
                if (item.group == "Bộ xử lý"){
                    var cpu = ""
                    for atrribute in item.attributes {
                        if(atrribute.name == "Hãng CPU"){
                            cpu = atrribute.value
                        }
                        if(atrribute.name == "Công nghệ CPU"){
                            cpu = "\(cpu) \(atrribute.value)"
                        }
                        if(atrribute.name == "Loại CPU"){
                            cpu = "\(cpu)-\(atrribute.value)"
                        }
                        if(atrribute.name == "Tốc độ CPU"){
                            cpu = "\(cpu) (\(atrribute.value))"
                        }
                    }
                    dictionaryProduct.updateValue(cpu, forKey: "1_CPU")
                }
                if (item.group == "RAM"){
                    var ram = ""
                    for atrribute in item.attributes {
                        if(atrribute.name == "Dung lượng RAM"){
                            ram = atrribute.value
                        }
                        if(atrribute.name == "Loại RAM"){
                            ram = "\(ram), \(atrribute.value)"
                        }
                        if(atrribute.name == "Tốc độ BUS RAM"){
                            ram = "\(ram) (\(atrribute.value))"
                        }
                    }
                    dictionaryProduct.updateValue(ram, forKey: "2_RAM")
                }
                if (item.group == "Đĩa cứng"){
                    var hdd = ""
                    for atrribute in item.attributes {
                        if(atrribute.name == "Dung lượng ổ đĩa"){
                            hdd = atrribute.value
                        }
                        if(atrribute.name == "Loại ổ đĩa"){
                            hdd = "\(hdd) \(atrribute.value)"
                        }
                    }
                    dictionaryProduct.updateValue(hdd, forKey: "3_HDD")
                }
                if (item.group == "Đồ họa"){
                    var vga = ""
                    for atrribute in item.attributes {
                        if(atrribute.name == "Chipset đồ họa "){
                            vga = atrribute.value
                        }
                        if(atrribute.name == "Bộ nhớ đồ họa"){
                            vga = "\(vga) \(atrribute.value)"
                        }
                    }
                    dictionaryProduct.updateValue(vga, forKey: "4_VGA")
                }
                if (item.group == "Màn hình"){
                    var screen = ""
                    for atrribute in item.attributes {
                        if(atrribute.name == "Kích thước màn hình"){
                            screen = atrribute.value
                        }
                    }
                    dictionaryProduct.updateValue(screen, forKey: "5_SCREEN")
                }
                if (item.group == "Hệ điều hành, phầm mềm có sẵn"){
                    var os = ""
                    for atrribute in item.attributes {
                        if(atrribute.name == "Hệ điều hành"){
                            os = atrribute.value
                        }
                    }
                    dictionaryProduct.updateValue(os, forKey: "6_OS")
                }
                if (item.group == "Kích thước & trọng lượng"){
                    var weight = ""
                    for atrribute in item.attributes {
                        if(atrribute.name == "Trọng lượng"){
                            weight = atrribute.value
                        }
                    }
                    dictionaryProduct.updateValue(weight, forKey: "7_WEIGHT")
                }
            }
        }
        var dictionaryProductCompare =  [String:String]()
        if(compareProduct.atrribute.count > 0){
            for item in compareProduct.atrribute {
                if (item.group == "Bộ xử lý"){
                    var cpu = ""
                    for atrribute in item.attributes {
                        if(atrribute.name == "Hãng CPU"){
                    ...
                    dictionaryProductCompare.updateValue(cpu, forKey: "1_CPU")
                }
                if (item.group == "RAM"){
                    var ram = ""
                    for atrribute in item.attributes {
                    ...
                    dictionaryProductCompare.updateValue(ram, forKey: "2_RAM")
                }
                if (item.group == "Đĩa cứng"){
                    var hdd = ""
                    for atrribute in item.attributes {
                    ...
                    dictionaryProductCompare.updateValue(hdd, forKey: "3_HDD")
                }
                if (item.group == "Đồ họa"){
                    var vga = ""
                    for atrribute in item.attributes {
                    ...
                    dictionaryProductCompare.updateValue(vga, forKey: "4_VGA")
                }
                if (item.group == "Màn hình"){
                    var screen = ""
                    for atrribute in item.attributes {
                    ...
                    dictionaryProductCompare.updateValue(screen, forKey: "5_SCREEN")
                }
                if (item.group == "Hệ điều hành, phầm mềm có sẵn"){
                    var os = ""
                    for atrribute in item.attributes {
                    ...
                    dictionaryProductCompare.updateValue(os, forKey: "6_OS")
                }
                if (item.group == "Kích thước & trọng lượng"){
                    var weight = ""
                    for atrribute in item.attributes {
                    ...
                    dictionaryProductCompare.updateValue(weight, forKey: "7_WEIGHT")
                }
            }
        }
        print(dictionaryProductCompare)
        print(dictionaryProduct)
        print(dictionaryProductCompare.count)
        print(dictionaryProduct.count)
        if(dictionaryProduct.count == dictionaryProductCompare.count){
             var indxGroup:Int = 0
            for item in dictionaryProduct.sorted(by: { $0.0 < $1.0 }){
                let itemCom = dictionaryProductCompare["\(item.key)"]
                var name = ""
                if(item.key == "1_CPU"){
                    name = "CPU"
                }else if(item.key == "2_RAM"){
                    name = "RAM"
                }
                else if(item.key == "3_HDD"){
                    name = "Bộ nhớ"
                }
                else if(item.key == "4_VGA"){
                    name = "VGA"
                }
                else if(item.key == "5_SCREEN"){
                    name = "Màn hình"
                }
                else if(item.key == "6_OS"){
                    name = "Hệ điều hành"
                }
                else if(item.key == "7_WEIGHT"){
                    name = "Trọng lượng"
                }
                ...
                let heightLb = "\(name)".height(withConstrainedWidth: viewInfoBasicRow.frame.size.width * 2/8 - Common.Size(s:10), font: UIFont.systemFont(ofSize: Common.Size(s:12)))
                ...
                lbBasicRow.text = "\(name)"
                lbBasicRow.numberOfLines = 4
                viewInfoBasicRow.addSubview(lbBasicRow)
                
                let heightValue1 = "\(item.value)".height(withConstrainedWidth: viewInfoBasicRow.frame.size.width * 3/8 - Common.Size(s:10), font: UIFont.systemFont(ofSize: Common.Size(s:12)))
                ...
                lbBasicValue1.text = "\(item.value.trimmingCharacters(in: CharacterSet.whitespacesAndNewlines))"
                lbBasicValue1.numberOfLines = 10
                lbBasicValue1.textAlignment = .center
                viewInfoBasicRow.addSubview(lbBasicValue1)
                
                let heightValue2 = "\(itemCom!)".height(withConstrainedWidth: viewInfoBasicRow.frame.size.width * 3/8 - Common.Size(s:10), font: UIFont.systemFont(ofSize: Common.Size(s:12)))
...
//            }
        ...
        if(listSPLenDoi.count == 2){
            ...
            btCompare2.addTarget(self, action: #selector(cartAction), for: .touchUpInside)
        ...
        if (self.compareProduct.product.qlSerial == "Y"){
            ...
            if (arrColor.count == 1){
                ...
                ActionSheetStringPicker.show(withTitle: "Chọn màu sản phẩm", rows: arrColor, initialSelection: 0, doneBlock: {
                    ...
                    let sku = self.compareProduct.variant[value].sku
                    let colorProduct = self.compareProduct.variant[value].colorValue
                    let priceBeforeTax = self.compareProduct.variant[value].priceBeforeTax
                    let price = self.compareProduct.variant[value].price
                    let product = self.compareProduct.product.copy() as! Product
