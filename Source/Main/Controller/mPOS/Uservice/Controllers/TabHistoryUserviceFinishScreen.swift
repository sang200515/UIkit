//
//  TabHistoryUserviceFinishScreen.swift
//  fptshop
//
...
class TabHistoryUserviceFinishScreen: BaseController {
    ...
    private var listHistoryUserviceFinish: [HistoryUserviceTickeData] = []
    ...
    @objc private func textFieldDidChange(_ textField: UITextField) {
        ...
        if isPickedDate == true && !listItemAfterFilteredByText.isEmpty {
            ...
            listItemAfterFilteredByText = listHistoryUserviceFinish.filter{$0.ticketOwner?.range(of: keyString, options: .caseInsensitive) != nil || $0.service?.range(of: keyString, options: .caseInsensitive) != nil || $0.id == searchNumber}
            numberChoose = 1
            tableView.reloadData()
        }
    ...
    func getListHistoryUservice() {
        ...
        UserviceAPIManager.shared.getListHistoryUservice(params: parameters, completion: { [weak self] (item, msg) in
            ...
            strongSelf.listHistoryUserviceFinish = item
        }) { (error) in
    ...
    func getDatesPicked(_ dates: [String]) {
        ...
        let item = listHistoryUserviceFinish.filter { items in
    ...
    func getAllItemsInMonths(_ items: [HistoryUserviceTickeData]) {
        ...
        listHistoryUserviceFinish.removeAll()
        let itemAfterFiltered = items.filter{ items in
            return allDays.contains { (date) -> Bool in
                items.createdTime?.range(of: date, options: .caseInsensitive) != nil
            }
        }
        listHistoryUserviceFinish = itemAfterFiltered
        tableView.reloadData()
    }
}

extension TabHistoryUserviceFinishScreen: UITableViewDelegate, UITableViewDataSource {
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        
        switch numberChoose {
        case 0:
            return listHistoryUserviceFinish.count
        case 1:
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        switch numberChoose {
        case 0:
            let item = listHistoryUserviceFinish[indexPath.row]
            cell.loadCell(item)
        case 1:
...
extension TabHistoryUserviceFinishScreen: PickerDateForUserviceViewDelegate {
