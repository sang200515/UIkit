//
//  CreateErrNetworkScreen.swift
//  fptshop
//
//  Created by KhanhNguyen on 11/12/20.
//  Copyright © 2020 FPT DIGITAL RETAIL JOINT STOCK COMPANY. All rights reserved.
//

import UIKit

class CreateErrNetworkScreen: BaseController {
    ...
    override func setupViews() {
        super.setupViews()
        self.title = "TẠO YÊU CẦU XỬ LÝ SỰ CỐ MẠNG"
        setupStackView()
    ...
    fileprivate func setupStackView() {
        ...
        let attributedForStarKey = [ NSAttributedString.Key.foregroundColor: Constants.COLORS.main_red_my_info ]
        let attributedStringNoteStar = NSMutableAttributedString(string: "(*):", attributes: attributedForStarKey)
        
        let myAttributeTitle = [ NSAttributedString.Key.font: UIFont.boldCustomFont(ofSize: 14) ]
        let titleDescription = NSMutableAttributedString(string: "Mô tả yêu cầu ", attributes: myAttributeTitle)
        
        titleDescription.append(attributedStringNoteStar)
        lblTitleDescribeTheRequest.attributedText = titleDescription
        ...
        let mainStackViewContainer = UIStackView(arrangedSubviews: [stackViewDescribeTheRequest, stackViewInsertIP])
    ...
    private func setupNoted() {
        ...
        let tapLabelLink = UITapGestureRecognizer(target: self, action: #selector(CreateErrNetworkScreen.tapToLink(sender:)))
    ...
    fileprivate func createTicket() {
        ...
        let params: [String:Any] = [
            "email": name,
            "token": token,
            "processKey": "XuLySuCoMang",
            "title": "XỬ LÝ SỰ CỐ MẠNG",
            "locationId": 5,
            "ticketOwner": name,
            "informedUsers": name,
            "values": "<item><line Name=\"NguoiYeuCau\" Value=\"\(name)\" /><line Name=\"MoTaYeuCau\" Value=\"\(contentResponse ?? "")\" /><line Name=\"NhapIP3G\" Value=\"\(iPValue ?? "")\" /></item>",
            "fromSystem": "mPOS"
    ...
    private func verifyWhileTyping(test: String) -> Bool {
...
extension CreateErrNetworkScreen: UITextFieldDelegate {
...
extension CreateErrNetworkScreen: UITextViewDelegate {
    func textViewDidChange(_ textView: UITextView) {
        placeholderLabel.isHidden = !textView.text.isEmpty
        self.contentResponse = textView.text
        if contentResponse?.count ?? 0 > 0 {
...
extension CreateErrNetworkScreen: PopAlerCreateIPScreenDelegate {
    func tappedOk() {
