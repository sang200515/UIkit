...
class PromotionViewController: UIViewController,UITableViewDelegate, UITableViewDataSource, ChangePromotionViewControllerDelegate {
    ...
    override func viewDidLoad() {
        ...
        self.promotions = Cache.promotions
        self.group = Cache.group
        var list: [PromotionsObject] = []
        for itemCart in Cache.carts {
            itemCart.product.priceSauKM = 0
        }
                ...
                if(items.Nhom == item.group){
                    ...
                    if(!checkSPMua || !checkSPTang){
                        if(items.SL_ThayThe > 0){
                            ...
                            item.promotions?.append(proArr)//3
                        }
                    }
                    check = true
                }
        ...
        Cache.resultPromostions.removeAll()
        Cache.resultPromostions.append(contentsOf: list)
        self.setupUI()
    }
    ...
    func setupUI(){
        ...
        let lbGroupPromotion = UILabel(frame: CGRect(x: 0, y: Common.Size(s:10), width: self.view.frame.size.width, height: Common.Size(s:18)))
        lbGroupPromotion.textAlignment = .center
        lbGroupPromotion.textColor = UIColor(netHex:0x04AB6E)
        lbGroupPromotion.font = UIFont.boldSystemFont(ofSize: Common.Size(s:16))
        lbGroupPromotion.text = "CHỌN NHÓM KHUYẾN MÃI"
        self.view.addSubview(lbGroupPromotion)
        
        tableView.frame = CGRect(x: 0, y:lbGroupPromotion.frame.origin.y + lbGroupPromotion.frame.size.height + Common.Size(s:10), width: self.view.frame.size.width, height: self.view.frame.size.height - Common.Size(s: 80) - ((navigationController?.navigationBar.frame.size.height)! + UIApplication.shared.statusBarFrame.height) - lbGroupPromotion.frame.origin.y - lbGroupPromotion.frame.size.height - Common.Size(s:10))
  ...
  private  func getPricegoiBH(itemBH:Int,endIndex:Int,itemEndIndex:Int){
                   ...
                   if item.product.amountGoiBH == "1" {
                    ...
                    if itemBH != 0 {
                            for i in 1...itemBH{
                                ...
                                let itemInsuraceCode:String = Cache.carts[index + i].product.itemCodeGoiBH
                                let imei = Cache.carts[index].imei
                                let priceMainProduct:Int =  Int(Cache.carts[index].product.price) - Int(Cache.carts[index].product.priceSauKM) - Cache.carts[index].discount
                                MPOSAPIManager.getPriceGoiBH(IMEI: imei, InsuranceCode: itemInsuraceCode, priceMainProduct: "\(priceMainProduct)", completion: { [weak self](results, err) in
                                    ...
                                    if index ==  endIndex {
                                        ...
                                        if results?.message?.message_Code == 200 && results?.data?.canSell == true {
                                            //có data
                                            let itemNameBH = Cache.carts[index + i].product.itemNameGoiBH
                                            let itemCodeBH = Cache.carts[index + i].product.itemCodeGoiBH
                                            let itemBrandBH = Cache.carts[index + i].product.brandName
            
                                            let price = results?.data?.insurancePrice ?? 0
                                            let pro = Product(model_id: "", id: 0, name: itemNameBH, brandID: 0, brandName: itemBrandBH, typeId: 0, typeName: Cache.carts[index + i].product.typeName, sku: "", price: Float(Int(price * 1.1)), priceMarket: 0, priceBeforeTax: 0, iconUrl: "", imageUrl: "", promotion: "", includeInfo: "", hightlightsDes: "", labelName: "", urlLabelPicture: "", isRecurring: false, manSerNum: "", bonusScopeBoom: "", qlSerial: "", inventory: 0, LableProduct: "", p_matkinh: "", ecomColorValue: "", ecomColorName: "", ecom_itemname_web: "", price_special: 0, price_online_pos: 0, price_online: 0, hotSticker: false, is_NK: false, is_ExtendedWar: false, skuBH: [], nameBH: [],brandGoiBH:[], isPickGoiBH: "0đ", amountGoiBH: "true", itemCodeGoiBH: itemCodeBH, itemNameGoiBH: itemNameBH ,priceSauKM: 0,role2: [])
            
                                            let cart = Cart(sku: itemCodeBH, product: pro,quantity: 1,color:"#ffffff",inStock:-1, imei: "",price: 0, priceBT: 0, whsCode: "", discount: 0,reason:"",note:"", userapprove: "", listURLImg: [], Warr_imei: results?.data?.imei ?? "", replacementAccessoriesLabel: "")
                                            Cache.carts[index + i] = cart
                                        }else {
        ...
        if groupChoose.count > 0 || notPromotionValue{
            for item in self.result{
                if(item.group == groupChoose){
                    ...
                    for itm in item.promotions!{
                        Cache.itemsPromotion.append(itm.promotionsMain)
                    }
                    break
                }
            }
            ...
            for item in Cache.itemsPromotion{
                debugPrint("EEEE \(item.SanPham_Mua) \(item.SanPham_Tang)")
                let it = item
                if (it.TienGiam <= 0){
                    if (promos.count == 0){
                        promos.append(it)
            ...
            newViewController.orderPayType = Cache.orderPayType
            newViewController.orderPayInstallment = Cache.orderPayInstallment
            newViewController.valueInterestRate = Cache.valueInterestRate
            newViewController.valuePrepay = Cache.valuePrepay
            newViewController.debitCustomer = Cache.debitCustomer
            newViewController.is_DH_DuAn = Cache.is_DH_DuAn
            self.navigationController?.pushViewController(newViewController, animated: true)
            
        }else{
            let alert = UIAlertController(title: "CHÚ Ý", message: "Bạn chưa chọn nhóm khuyến mãi!", preferredStyle: .alert)
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if (item.promotionsMain.TienGiam > 0){
            for itemC in Cache.carts{
                if item.promotionsMain.SanPham_Mua == itemC.sku && itemC.product.typeName != "Goi1" && itemC.product.typeName != "Goi2"{
                    itemC.product.priceSauKM += item.promotionsMain.TienGiam
                }
            }
            price = "Giảm giá: \(Common.convertCurrencyFloat(value: item.promotionsMain.TienGiam))"
        ...
        cell.note.text = "(Khi mua: \(item.promotionsMain.TenSanPham_Mua)) - (Mã cơ cấu): \(item.promotionsMain.MaCTKM)"
    ...
    func tableView(_ tableView: UITableView, viewForHeaderInSection section: Int) -> UIView? {
        ...
        let  radioPayNow = createRadioButtonGroup(CGRect(x: Common.Size(s:10),y:0 , width: tableView.frame.size.width/2, height: Common.Size(s:44)), title: "\(self.result[section].group)", color: UIColor(netHex:0x04AB6E));
        if (groupChoose == "\(self.result[section].group)"){
            radioPayNow.isSelected = true
        }else if (groupChoose == ""){
            
            if(Cache.is_DH_DuAn == "N"){
    ...
    func updateTotal(discountPrice: Float){
        ...
        for item in Cache.carts {
            if item.product.itemCodeGoiBH != "" {
                ...
                sum = sum + item.quantity * Int(item.product.price)
                discount = discount + item.discount
            }
        }
        sum = sum - Int(discountPrice) - discount
        lbTotalValue.text = Common.convertCurrency(value: sum)
    }
}
class ItemGiftTableViewCellV2: UITableViewCell {
    ...
    override init(style: UITableViewCell.CellStyle, reuseIdentifier: String?) {
        ...
        title.textColor =  UIColor(netHex:0xEF4A40)
        ...
        quanlity.leadingAnchor.constraint(equalTo: marginGuide.leadingAnchor).isActive = true
        ...
        quanlity.font = UIFont.systemFont(ofSize: Common.Size(s:14))
        quanlity.textColor = UIColor.black
        
        contentView.addSubview(quanlityChange)
        quanlityChange.translatesAutoresizingMaskIntoConstraints = false
        quanlityChange.leadingAnchor.constraint(equalTo: marginGuide.leadingAnchor).isActive = true
        quanlityChange.trailingAnchor.constraint(equalTo: marginGuide.trailingAnchor).isActive = true
        quanlityChange.topAnchor.constraint(equalTo: quanlity.bottomAnchor).isActive = true
        quanlityChange.numberOfLines = 0
        quanlityChange.font = UIFont.systemFont(ofSize: Common.Size(s:14))
        quanlityChange.textColor = UIColor(netHex:0x04AB6E)
        
        contentView.addSubview(note)
        note.translatesAutoresizingMaskIntoConstraints = false
        note.leadingAnchor.constraint(equalTo: marginGuide.leadingAnchor).isActive = true
        note.bottomAnchor.constraint(equalTo: marginGuide.bottomAnchor).isActive = true
        note.trailingAnchor.constraint(equalTo: marginGuide.trailingAnchor).isActive = true
        note.topAnchor.constraint(equalTo: quanlityChange.bottomAnchor).isActive = true
        note.numberOfLines = 0
        note.font = UIFont.systemFont(ofSize: Common.Size(s:10))
        note.textColor =  UIColor.gray
        
    }
}
extension PromotionViewController : FZAccordionTableViewDelegate {
