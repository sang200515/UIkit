...
protocol ScanIMEIControllerDelegate: NSObjectProtocol {
    
    func scanSuccess(_ viewController: ScanIMEIController, scan: String)
    
}

class ScanIMEIController: UIViewController, AVCaptureMetadataOutputObjectsDelegate {
    
    weak var delegate: ScanIMEIControllerDelegate?
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        self.navigationItem.title = "Quét IMEI"
        success = false
        
        let btRightIcon = UIButton.init(type: .custom)
        btRightIcon.setImage(#imageLiteral(resourceName: "list"), for: UIControl.State.normal)
        btRightIcon.imageView?.contentMode = .scaleAspectFit
        btRightIcon.addTarget(self, action: #selector(ScanIMEIController.actionSelectImei), for: UIControl.Event.touchUpInside)
    ...
    @objc func actionSelectImei(){
        _ = navigationController?.popViewController(animated: true)
        dismiss(animated: true, completion: nil)
        delegate?.scanSuccess(self, scan: "SELECT_IMEI")
    }
