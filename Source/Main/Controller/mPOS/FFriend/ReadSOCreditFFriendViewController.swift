//
//  ReadSOCreditFFriendViewController.swift
//  fptshop
//
...
import PopupDialog
class ReadSOCreditFFriendViewController: UIViewController,UITextFieldDelegate,UITextViewDelegate{
    var carts:[Cart] = []
    ...
    override func viewDidLoad() {
        ...
        btLeftIcon.addTarget(self, action: #selector(PromotionFFriendViewController.actionOpenMenuLeft), for: UIControl.Event.touchUpInside)
    ...
    func setupUI() {
        ...
        let lbTextCode = UILabel(frame: CGRect(x: Common.Size(s:20), y: radioPayNow.frame.size.height + radioPayNow.frame.origin.y + Common.Size(s:10), width: scrollView.frame.size.width - Common.Size(s:40), height: Common.Size(s:14)))
    ...
    func tapShowDetailTypePay(sender:UITapGestureRecognizer) {
    ...
    @objc func actionPay(sender: UIButton!) {
        if (buttonSaveAction == false){
            ...
            if(isImei == false){
                let title = "THÔNG BÁO"
                let message = "Đơn hàng của bạn bắt buộc phải có ít nhất 1 sản phẩm có IMEI."
                ...
                let buttonTow = CancelButton(title: "OK") {
                }
            ...
            if (sumMoney < 3000000){
                let title = "THÔNG BÁO"
                let message = "Tổng tiền đơn hàng Credit không được nhỏ hơn 3.000.000đ"
                let popup = PopupDialog(title: title, message: message, buttonAlignment: .horizontal, transitionStyle: .zoomIn, tapGestureDismissal: false, panGestureDismissal: false, hideStatusBar: false) {
                }
                let buttonTow = CancelButton(title: "OK") {
                }
            ...
            if(isConntectStatus != .notReachable){
                ...
                let thanhTien = "\(String(format:"%.2f", sumMoney))"
                var soTienTraTruoc = "0"
                if(Cache.ocfdFFriend!.LoaiKH == 5){
                    if(sumMoney > Cache.ocfdFFriend!.HanMucConLai){
                        soTienTraTruoc =  "\(String(format:"%.2f", (sumMoney - Cache.ocfdFFriend!.HanMucConLai)))"
                    }
                }
                
                MPOSAPIManager.mpos_sp_insert_order_ffriend(cmnd: "\(Cache.ocfdFFriend!.CMND)", CardName: "\(Cache.ocfdFFriend!.CardName)", U_EplCod: "\(Cache.user!.UserName)", ShopCode: "\(Cache.user!.ShopCode)", Sotientratruoc: "\(soTienTraTruoc)", Doctype: "\(Cache.typeOrder)", U_des: "", RDR1: rDR1, PROMOS: pROMOS, LoaiTraGop: "0", LaiSuat: "0", voucher: "", otp: "", NgayDenShopMua: datePay, HinhThucGH: "\(self.typeCode!)", DiaChi: "\(address)", magioithieu: code, kyhan: "0", Thanhtien: "\(thanhTien)", IDcardcode: "\(Cache.ocfdFFriend!.IDcardCode)", HinhThucThuTien: "\(hinhThucThuTien)", ListSP: "\(skuList)", IsSkip: "", AuthenBy: "",TransactionID:Cache.ocfdFFriend!.TransactionID,p_docentry: "\(Cache.depositEcomNumFF)", handler: { (results,IdFinal,IdFromDK,IsExpired_CL_UQTN, err)  in
