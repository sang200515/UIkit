//
//  DetailProductNewViewController.swift
//  fptshop
//
...
class DetailProductNewViewController: UIViewController,UICollectionViewDelegateFlowLayout, UICollectionViewDataSource {
    ...
    override func viewDidLoad() {
        ...
        btBackIcon.addTarget(self, action: #selector(DetailProductNewViewController.actionBack), for: UIControl.Event.touchUpInside)
        ...
        btCartIcon.addTarget(self, action: #selector(DetailProductNewViewController.actionCart), for: UIControl.Event.touchUpInside)
    ...
    func loadProduct(product:ProductBySku) {
        ...
        let sizeTitle = "\(product.product.promotion.stripHTML().stringByDecodingHTMLEntities.replace(target: "\t", withString:"- ").trimmingCharacters(in: CharacterSet.whitespacesAndNewlines))".height(withConstrainedWidth: viewInfoPromotion.frame.size.width - Common.Size(s:10), font: UIFont.systemFont(ofSize: Common.Size(s:12)))
        ...
        lbPromotion.text = product.product.promotion.stripHTML().stringByDecodingHTMLEntities.replace(target: "\t", withString:"- ").trimmingCharacters(in: CharacterSet.whitespacesAndNewlines)
        ...
        let tap = UITapGestureRecognizer(target: self, action: #selector(DetailProductNewViewController.tapShowDetail))
