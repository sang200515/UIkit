//
//  UpdateCustomerPayDirectlyViewController.swift
//  fptshop
//
...
import SwiftyBeaver
class UpdateCustomerPayDirectlyViewController: UIViewController,UITextFieldDelegate,UITextViewDelegate {
    ...
    override func viewDidLoad() {
        ...
        self.companyButton.filterStrings([])
        ...
        self.branchCompanyButton.filterStrings([])
        ...
        let lbTextPosition = UILabel(frame: CGRect(x: Common.Size(s:15), y: tfCMND.frame.size.height + tfCMND.frame.origin.y + Common.Size(s:5), width: scrollView.frame.size.width - Common.Size(s:30), height: Common.Size(s:14)))
        ...
        tfEmail = UITextField(frame: CGRect(x: Common.Size(s:15), y: lbTextEmail.frame.size.height + lbTextEmail.frame.origin.y + Common.Size(s:5), width: scrollView.frame.size.width/2 - Common.Size(s:15) , height: Common.Size(s:40)))
        tfEmail.placeholder = ""
        ...
        tfEmail.placeholder = "Chỉ nhập tên email"
        detailView.addSubview(tfEmail)
        
        emailButton = SearchTextField(frame: CGRect(x: tfEmail.frame.origin.x + tfEmail.frame.size.width, y: tfEmail.frame.origin.y , width: tfEmail.frame.size.width , height: tfEmail.frame.size.height ));
        ...
        self.emailButton.filterStrings([])
        ...
        let lbTextNote = UILabel(frame: CGRect(x: Common.Size(s:15), y:tfEmail.frame.size.height + tfEmail.frame.origin.y + Common.Size(s:5), width: scrollView.frame.size.width - Common.Size(s:30), height: Common.Size(s:14)))
        ...
        detailView.addSubview(lbTextNote)
        
        taskNotes = UITextView(frame: CGRect(x: lbTextNote.frame.origin.x , y: lbTextNote.frame.origin.y  + lbTextNote.frame.size.height + Common.Size(s:10), width: lbTextNote.frame.size.width, height: tfEmail.frame.size.height * 2 ))
        ...
        detailView.addSubview(taskNotes)
...
//        let lbTextHT = UILabel(frame: CGRect(x: Common.Size(s:15), y:taskNotes.frame.size.height + taskNotes.frame.origin.y + Common.Size(s:5), width: scrollView.frame.size.width - Common.Size(s:30), height: Common.Size(s:14)))
...
//
//        radOTP = createRadioButtonHT(CGRect(x: lbTextHT.frame.origin.x,y:lbTextHT.frame.origin.y + lbTextHT.frame.size.height + Common.Size(s:10) , width: lbTextHT.frame.size.width, height: Common.Size(s:15)), title: "Thẩm định qua Mail công ty của KH (OTP)", color: UIColor.black);
//        detailView.addSubview(radOTP)
//        radOTP.isSelected = true
//        htType = 1
//
//        radCallLog = createRadioButtonHT(CGRect(x: lbTextHT.frame.origin.x ,y:radOTP.frame.origin.y + radOTP.frame.size.height + Common.Size(s:10), width: radOTP.frame.size.width, height: radOTP.frame.size.height), title: "Thẩm định qua CallLog (Duyệt thủ công)", color: UIColor.black);
...
//        self.heightView =  viewOTP.frame.size.height
        ...
        btSaveCustomer.frame = CGRect(x: tfEmail.frame.origin.x, y: taskNotes.frame.origin.y + taskNotes.frame.size.height + Common.Size(s:20), width: tfName.frame.size.width, height: tfEmail.frame.size.height * 1.2)
        ...
        if (ocfdFFriend != nil){
            ...
            tfDateBirthday.text = ocfdFFriend?.Birthday
            ...
            tfDate.text = dateCom
            let listEmail = ocfdFFriend?.Email.components(separatedBy: "@")
            if(listEmail!.count > 1){
                tfEmail.text = listEmail![0]
                emailButton.text = "@\(listEmail![1])"
            }else{
    ...
    @objc func actionSendOTP(){
        ...
        MPOSAPIManager.mpos_CheckEmail_Vendor_SendOTP(vendorcode:"\(self.vendorCode)",idcard:"\(self.tfCMND.text!)",phonenumber:"\(self.tfPhone.text!)",email:"\(emailFull)",idcardcode:"\(self.ocfdFFriend!.IDcardCode)") { (stauts,message,err) in
            nc.post(name: Notification.Name("dismissLoading"), object: nil)
            let when = DispatchTime.now() + 0.5
            DispatchQueue.main.asyncAfter(deadline: when) {
                if(err.count <= 0){
    ...
    func imageCMNDTruoc(image:UIImage){
        ...
        detailView.frame.origin.y = viewUpload.frame.size.height + viewUpload.frame.origin.y + Common.Size(s:20)
        scrollView.contentSize = CGSize(width:UIScreen.main.bounds.size.width, height: detailView.frame.origin.y + detailView.frame.size.height + (navigationController?.navigationBar.frame.size.height)! + UIApplication.shared.statusBarFrame.height + Common.Size(s:20))
...
//        }
                             ...
                             if(result!.IdCard == self.tfCMND.text!){
                                ...
                                self.cmnDetect = result!.IdCard
                                 ...
                                 let buttonOne = CancelButton(title: "OK") {
                                    ...
                                    self.viewInfoTheNV.frame.origin.y = self.viewInfoCMNDSau.frame.origin.y + self.viewInfoCMNDSau.frame.size.height + Common.Size(s:10)
    ...
    @objc func actionCompleteImage(){
        ...
        MPOSAPIManager.sp_mpos_UploadImageScoring(UserID:Cache.user!.UserName,IdCardCode:"0",
                                                  ...
                                                  Url_SaoKeLuong_2:"",
                                                  Url_SaoKeLuong_3:"",
                                                  CMND: self.tfCMND.text!,VendorCode: "\(self.vendorCode)", handler: { (result, err) in
                                                    let when = DispatchTime.now() + 0.5
                                                    DispatchQueue.main.asyncAfter(deadline: when) {
                                                         nc.post(name: Notification.Name("dismissLoading"), object: nil)
                                                        if(err.count <= 0){
    ...
    func actionSaveCustomer(IDImageXN:String){
        ...
        let emailFull = "\(email)\(emailButton.text!)"
      
        
        let newViewController = LoadingViewController()
        newViewController.content = "Đang cập nhật thông tin KH..."
        newViewController.modalPresentationStyle = UIModalPresentationStyle.overCurrentContext
        newViewController.modalTransitionStyle = UIModalTransitionStyle.crossDissolve
        self.navigationController?.present(newViewController, animated: true, completion: nil)
        let nc = NotificationCenter.default
        
        MPOSAPIManager.AddCustomerFFriend(VendorCode: "\(ocfdFFriend!.MaCongTy)", CMND: cmnd, FullName: name, SDT: phone, HoKhauTT: "\(ocfdFFriend!.DiaChiHoKhau)", NgayCapCMND: "\(ocfdFFriend!.NgayCapCMND)", NoiCapCMND: "\(ocfdFFriend!.NoiCapCMND)", ChucVu: "\(self.positionCode)", NgayKiHD: "\(self.tfDate.text!)", SoTKNH: "\(ocfdFFriend!.SoTKNH)", IdBank: "\(ocfdFFriend!.IdBank)", ChiNhanhNH: "\(ocfdFFriend!.ChiNhanhNH)", Email: emailFull, Note: "\(note)", CreateBy: "\(Cache.user!.UserName)", FileAttachName: "", ChiNhanhDN: "\(ocfdFFriend!.MaChiNhanhDoanhNghiep)",IDCardCode: "\(self.ocfdFFriend!.IDcardCode)", LoaiKH: "2", NgaySinh: "\(tDateBirthday)", CreditCard: "", MaNV_KH: "", VendorCodeRef: "", CMND_TinhThanhPho: "0", CMND_QuanHuyen: "0", CMND_PhuongXa: "", NguoiLienHe: "", SDT_NguoiLienHe: "", QuanHeVoiNguoiLienHe: "0", NguoiLienHe_2: "", SDT_NguoiLienHe_2: "", QuanHeVoiNguoiLienHe_2: "0", AnhDaiDien: "",GioThamDinh_TimeFrom:"",GioThamDinh_TimeTo:"",GioThamDinh_OtherTime:"",IdCardcodeRef:"",TenSPThamDinh:"",Gender: -1,IDImageXN:"\(IDImageXN)",ListIDSaoKe:"",NguoiLienHe_3:"",SDT_NguoiLienHe_3:"",QuanHeVoiNguoiLienHe_3:"",NguoiLienHe_4:"",SDT_NguoiLienHe_4:"",QuanHeVoiNguoiLienHe_4:"",EVoucher:"",SoHopDong:"",isQrCode:"",isComplete:"",OTP:"") { (result,code, err) in
            nc.post(name: Notification.Name("dismissLoading"), object: nil)
            let when = DispatchTime.now() + 0.5
            DispatchQueue.main.asyncAfter(deadline: when) {
                 nc.post(name: Notification.Name("dismissLoading"), object: nil)
    ...
    func tapCreateCredit(sender:UITapGestureRecognizer) {
    ...
    fileprivate func createRadioButtonHT(_ frame : CGRect, title : String, color : UIColor) -> DLRadioButton {
          ...
          radioButton.addTarget(self, action: #selector(UpdateCustomerPayDirectlyViewController.logSelectedButtonHT), for: UIControl.Event.touchUpInside);
      ...
      @objc @IBAction fileprivate func logSelectedButtonHT(_ radioButton : DLRadioButton) {
          if (!radioButton.isMultipleSelectionEnabled) {
              ...
              switch temp {
              case "Thẩm định qua Mail công ty của KH (OTP)":
                  htType = 1
...
extension UpdateCustomerPayDirectlyViewController: UINavigationControllerDelegate, UIImagePickerControllerDelegate {
