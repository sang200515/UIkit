//
//  CustomerCreditViewController.swift
//  fptshop
//
...
import Foundation
import PopupDialog
class CustomerCreditViewController: UIViewController,UITextFieldDelegate {
    ...
    override func viewDidLoad() {
        ...
        //        tfDateBirthday.borderStyle = UITextBorderStyle.roundedRect
        //        tfDateBirthday.autocorrectionType = UITextAutocorrectionType.no
        //        tfDateBirthday.keyboardType = UIKeyboardType.default
        //        tfDateBirthday.returnKeyType = UIReturnKeyType.done
        //        tfDateBirthday.clearButtonMode = UITextFieldViewMode.whileEditing;
        //        tfDateBirthday.contentVerticalAlignment = UIControlContentVerticalAlignment.center
        ...
        btSaveCustomer.frame = CGRect(x: tfCMND.frame.origin.x, y: tfCMND.frame.size.height + tfCMND.frame.origin.y + Common.Size(s:20), width: tfName.frame.size.width, height: tfCMND.frame.size.height * 1.2)
        ...
        MPOSAPIManager.getVendorFFriend(LoaiDN: "4", handler: { (results, err) in
            ...
            if(err.count<=0){
                ...
                for item in results {
                    ...
                    if(self.codeIdBank != nil){
                        if("\(item.ID)" == self.codeIdBank){
    ...
    func checkCreateCustomer(){
        ...
        MPOSAPIManager.sp_mpos_CheckCreateCustomer(VendorCode: "\(self.vendorCode)", LoaiKH: "4", handler: { (result, message) in
            if(message.count > 0){
                let title = "Thông báo"
