...
class PaymentRightPhoneViewController: UIViewController,ChooseBankRightPhoneViewControllerDelegate,UITextFieldDelegate,UITextViewDelegate {
    ...
    override func viewDidLoad() {
        ...
        btLeftIcon.addTarget(self, action: #selector(PaymentRightPhoneViewController.backButton), for: UIControl.Event.touchUpInside)
        ...
        let lblNote = UILabel(frame: CGRect(x: Common.Size(s:10), y: lblShop.frame.size.height + lblShop.frame.origin.y +  Common.Size(s: 10), width: scrollView.frame.size.width - Common.Size(s: 250) , height: Common.Size(s:14)))
        ...
        let lblNoteValue = UILabel(frame: CGRect(x: lblNote.frame.size.width + lblNote.frame.origin.x + Common.Size(s:10), y: lblNote.frame.origin.y, width: scrollView.frame.size.width - Common.Size(s: 100), height: Common.Size(s:14)))
        lblNoteValue.textAlignment = .left
        lblNoteValue.textColor = UIColor.black
        lblNoteValue.font = UIFont.systemFont(ofSize: Common.Size(s:12))
        lblNoteValue.text = "\(self.detailAfterSaleRP!.Note)"
        ...
        lblNoteValue.frame = CGRect(x: lblNoteValue.frame.origin.x, y: lblNote.frame.origin.y, width: lblNoteValue.frame.width, height: lblNoteValueHeight)
        
        
        viewSeller.frame.size.height = lblNoteValue.frame.size.height + lblNoteValue.frame.origin.y + Common.Size(s:10)
        ...
        lblBuyerNameValue.text = "\(self.detailAfterSaleRP!.Buy_Name)"
        ...
        lblDateBuyValue.text = "\(self.detailAfterSaleRP!.buy_date)"
        ...
        lblCodeRightPhoneValue.text = "\(self.detailAfterSaleRP!.Buy_invoice_SKTel)"
        ...
        tfTypeCardBank.itemSelectionHandler = { filteredResults, itemPosition in
            ...
            self.tfCash.text = Common.convertCurrencyV2(value: Int(self.detailAfterSaleRP!.SoTienConLai))
            self.tfCard.text = ""
        }
        ...
        tfCash.text = "\(Common.convertCurrencyV2(value: Int(self.detailAfterSaleRP!.SoTienConLai)))"
...
//        viewTheDetail.addSubview(tfBankNumber)
        ...
        viewTheDetail.frame.size.height = tfCard.frame.size.height + tfCard.frame.origin.y + Common.Size(s:10)
    ...
    fileprivate func createRadioButtonLoaiThueBao(_ frame : CGRect, title : String, color : UIColor) -> DLRadioButton {
        ...
        radioButton.addTarget(self, action: #selector(PaymentRightPhoneViewController.logSelectedButtonLoaiThueBao), for: UIControl.Event.touchUpInside);
    ...
    @objc func actionComplete(){
        ...
        MPOSAPIManager.mpos_FRT_SP_SK_confirm_book_order(Type:"1",otp:"\(self.tfOTP.text!)",docentry:"\(self.itemRPOnProgress!.docentry)",xmlpay:self.parseXML(),note:self.tfNote.text!,imei:self.detailAfterSaleRP!.IMEI,price:"\(Int(self.detailAfterSaleRP!.Sale_price))",phone:"\(self.detailAfterSaleRP!.Buy_phone)") { (p_status,p_message, err) in
            let when = DispatchTime.now() + 0.5
            DispatchQueue.main.asyncAfter(deadline: when) {
                nc.post(name: Notification.Name("dismissLoading"), object: nil)
                if(err.count <= 0){
    ...
    @objc func textFieldDidChangeMoneyTienThe(_ textField: UITextField) {
        ...
        if(characters.count > 0){
            ...
            let tongTienWithoutFee = (Int(self.detailAfterSaleRP!.SoTienConLai) - Int(str)! )
           
            self.tfCash.text = Common.convertCurrencyV2(value: Int(tongTienWithoutFee) )
            if (self.tfCash.text?.hasPrefix("-"))! {
                ...
                alert.addAction(UIAlertAction(title: "Đồng ý", style: .default) { _ in
                    self.tfCard.text = ""
                    self.tfCash.text = Common.convertCurrencyV2(value: Int(self.detailAfterSaleRP!.SoTienConLai) + Int(self.feeAmount))
                })
                self.present(alert, animated: true)
            }
        }else{
        ...
        if(self.radCard.isSelected == true){
            if(self.tfCash.text != ""){
                ...
                if tienMatBanDau < 0 {
                    ...
                    rs = rs + "<item totalcash=\"\(0)\"  Totalcredit=\"\(moneyStringCard)\" numbercredit=\"\("\("")")\"  IDBank=\"\("\(self.bank!.code)")\"  typebank=\"\("\(self.itemTypeCard!.CreditCard)")\" feebank=\"\("\(Int(self.feeAmount))")\"/>"
            }else{
