...
import PopupDialog
import Toaster
public protocol SearchVNPTDelegate {
...
class SearchProdutionVNPTViewController: YNSearchViewController, YNSearchDelegate{
    func ynCategoryButtonClose() {
        self.navigationController?.popViewController(animated: false)
    }
    var btCartIcon:MIBadgeButton!
    var searchField: UITextField!
    var delegateSearchView:SearchVNPTDelegate?
    var isFFriend:Bool = false
    override func viewDidLoad() {
        super.viewDidLoad()
        self.view.backgroundColor = .white
        let demoCategories = ["iPhone Xs Max", "Samsung S20+", "Note 10", "Iphone 11"]
        ...
        btCartIcon.addTarget(self, action: #selector(SearchViewController.actionCart), for: UIControl.Event.touchUpInside)
                            ...
                            for item in result!.detailSPs{
                                ProductAPIManager.product_detais_by_sku(sku: item.ItemCode,handler: { (success , error) in
                                    if(success.count > 0){
                                        ...
                                        if(Cache.cartsVNPT.count == result!.detailSPs.count){
