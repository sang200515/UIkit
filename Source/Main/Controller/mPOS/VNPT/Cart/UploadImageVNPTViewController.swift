...
import PopupDialog
class UploadImageVNPTViewController:UIViewController,UITextFieldDelegate{
    ...
    override func viewDidLoad() {
        ...
        btLeftIcon.addTarget(self, action: #selector(UploadImageVNPTViewController.backButton), for: UIControl.Event.touchUpInside)
        ...
        tfCMND.text = Cache.infoCMNDVNPT!.CMND
    ...
    @objc func actionConfirm(){
        ...
        MPOSAPIManager.mpos_FRT_SP_VNPT_upload_anhKH(CMND:Cache.infoCMNDVNPT!.CMND,url_anhkh:self.url_hinh_kh,docentry:"\(Cache.infoCMNDVNPT!.Docentry)",sompos:"\(self.docentry!)") { (p_status,message,err) in
    ...
    func uploadImage(image:UIImage,type:String){
        ...
        if let imageData:NSData = image.jpegData(compressionQuality: Common.resizeImageScanCMND) as NSData?{
            ...
            MPOSAPIManager.mpos_FRT_Image_VNPT(CMND:"\(Cache.infoCMNDVNPT!.CMND)",IDMpos:"\(self.docentry!)",Base64:"\(strBase64)",Type:"4") { (result,err) in
                nc.post(name: Notification.Name("dismissLoading"), object: nil)
                let when = DispatchTime.now() + 0.5
                DispatchQueue.main.asyncAfter(deadline: when) {
                    if(err.count <= 0){
...
extension UploadImageVNPTViewController: UINavigationControllerDelegate, UIImagePickerControllerDelegate {
