//
//  DetailEsimViewController.swift
//  fptshop
//
//  Created by tan on 3/7/19.
...
import PopupDialog
import Presentr
class DetailEsimViewController: UIViewController,UITextFieldDelegate,EPSignatureDelegate,DialogCustomersDelegate,ScanBarcodeViewControllerDelegate {
    ...
    override func viewDidLoad() {
        ...
        let lbImageInfo = UILabel(frame: CGRect(x: Common.Size(s:20), y: 0, width: scrollView.frame.size.width - Common.Size(s:40), height: Common.Size(s:20)))
        ...
        let sizeCMNDT =  viewImageCMNDTruoc.frame.size.height * 2/3
        let viewCMNDTruocButton = UIImageView(frame: CGRect(x: viewImageCMNDTruoc.frame.size.width/2 - sizeCMNDT/2, y: 0, width: viewImageCMNDTruoc.frame.size.height * 2/3, height: viewImageCMNDTruoc.frame.size.height * 2/3))
        ...
        let sizeCMNDS = viewImageCMNDSau.frame.size.height * 2/3
        let viewCMNDSauButton = UIImageView(frame: CGRect(x: viewImageCMNDSau.frame.size.width/2 - sizeCMNDS/2, y: 0, width: viewImageCMNDSau.frame.size.height * 2/3, height: viewImageCMNDSau.frame.size.height * 2/3))
        ...
        //-------------------
        //address
        tfAddress = UITextField(frame: CGRect(x: tfBirthday.frame.origin.x, y: tfBirthday.frame.origin.y + tfBirthday.frame.size.height + Common.Size(s:10), width: tfBirthday.frame.size.width , height: tfBirthday.frame.size.height ));
        ...
        //--------------------
        //address
        cityButton = SearchTextField(frame: CGRect(x: tfAddress.frame.origin.x, y: tfAddress.frame.origin.y + tfAddress.frame.size.height + Common.Size(s:10), width: tfAddress.frame.size.width , height: tfAddress.frame.size.height ));
        ...
        let imageButton = UIImageView(frame: CGRect(x: tfUserName.frame.size.height/4, y: tfUserName.frame.size.height/4, width: tfAddress.frame.size.height/2, height: tfUserName.frame.size.height/2))
        ...
        let lbCMNDInfo = UILabel(frame: CGRect(x: Common.Size(s:20), y: nationalButton.frame.origin.y + nationalButton.frame.size.height + Common.Size(s:10), width: scrollView.frame.size.width - Common.Size(s:40), height: Common.Size(s:20)))
        ...
        tfSerialSim = UITextField(frame: CGRect(x: tfCMND.frame.origin.x, y: amountButton.frame.origin.y + amountButton.frame.size.height + Common.Size(s:10), width: tfCMND.frame.size.width - Common.Size(s:50), height: 0));
        ...
        tfSerialSim.isUserInteractionEnabled = false
        tfSerialSim.isEnabled = false
        //        tfUserName.addTarget(self, action: #selector(textFieldDidChangeName(_:)), for: .editingChanged)
        viewInfoBottom.addSubview(tfSerialSim)
        
        tfSerialSim.leftViewMode = UITextField.ViewMode.always
        let imageSerialSim = UIImageView(frame: CGRect(x: tfUserName.frame.size.height/4, y: tfUserName.frame.size.height/4, width: tfCMND.frame.size.height/2, height: 0))
        ...
        let sizeAVAT = viewImageAvatar.frame.size.height * 2/3
        
        let viewAvatarButton = UIImageView(frame: CGRect(x: viewImageAvatar.frame.size.width/2 - sizeAVAT/2, y: 0, width: viewImageAvatar.frame.size.height * 2/3, height: viewImageAvatar.frame.size.height * 2/3))
        ...
        let sizeSigt = viewImageSign.frame.size.height * 2/3
        let viewSignButton = UIImageView(frame: CGRect(x: viewImageSign.frame.size.width/2 - sizeSigt/2, y: 0, width: viewImageSign.frame.size.height * 2/3, height: viewImageSign.frame.size.height * 2/3))
                        ...
                        if(err.count <= 0){
                            if(results.count > 0){
                                ...
                                if (item.Gender == 0){
                                    ...
                                    self.radioFemale.isSelected = false
                                    self.gender = 1
                                }else if (item.Gender == 1){
                                    self.radioFemale.isSelected = true
                                    self.radioMale.isSelected = false
                                    self.gender = 2
                                }
                                ...
                                if(self.wardsButton.text == ""){
                                    let obj4 =  self.listPrecincts.filter{ $0.Text == "\(Precinct)" }.first
                                    if let _ = obj4?.Text {
                                        ...
                                        self.wardsButton.text = "\(Precinct)"
                                        self.selectPrecinct = "\(Precinct)"
                                    }
                                }
...
//        }
                ...
                if let imageDataCMNDSau:NSData = imageCMNDSau.jpegData(compressionQuality: Common.resizeImageValue)  as NSData?{
                    ...
                    if let imageDataAvatar:NSData = imageAvatar.jpegData(compressionQuality: Common.resizeImageValue) as NSData?{
                        ...
                        if (simActive!.Provider == "Viettel"){
                            ...
                            self.activeViettel(phoneNum: phoneNum ?? "", p_TenKH: name, gioiTinh: gt, birthday: birthday, nationalText: nationalText, addressVT: addressVT, serialSim: serialSim, strBase64Sign: strBase64Sign, strBase64CMNDTruoc: strBase64CMNDTruoc, strBase64CMNDSau: strBase64CMNDSau, strBase64Avatar: strBase64Avatar, inputCMND: inputCMND, inputDateCMND: inputDateCMND, inputAddressCMND: inputAddressCMND, inputAddressPassport: inputAddressPassport, inputSoVisa: inputSoVisa, contractNo: numSO, valuePassport: valuePassport, valueDatePassport: valueDatePassport)
                            ...
                            self.activeVinaphone(phoneNum: phoneNum ?? "", p_TenKH: name, gioiTinh: gt, birthday: birthday, nationalText: nationalText, address: address, serialSim: serialSim, strBase64Sign: strBase64Sign, strBase64CMNDTruoc: strBase64CMNDTruoc, strBase64CMNDSau: strBase64CMNDSau, strBase64Avatar: strBase64Avatar, inputCMND: inputCMND, inputDateCMND: inputDateCMND, inputAddressCMND: inputAddressCMND, inputAddressPassport: inputAddressPassport, inputSoVisa: inputSoVisa, contractNo: numSO, valuePassport: valuePassport, valueDatePassport: valueDatePassport)
                            ...
                            self.activeMobifone(phoneNum: phoneNum ?? "", p_TenKH: name, birthday: birthday, gioiTinh: gt, quocTich: qt, inputCMND: inputCMND, inputDateCMND: inputDateCMND, inputAddressCMNDPhieu: inputAddressCMNDPhieu, address: address, serialSim: serialSim, strBase64Sign: strBase64Sign, strBase64CMNDTruoc: strBase64CMNDTruoc, strBase64CMNDSau: strBase64CMNDSau, strBase64Avatar: strBase64Avatar, inputAddressCMND: inputAddressCMND, inputAddressPassport: inputAddressPassport, inputSoVisa: inputSoVisa, contractNo: numSO, valuePassport: valuePassport, valueDatePassport: valueDatePassport)
                            ...
                            self.activeVietnammobile(phoneNum: phoneNum ?? "", p_TenKH: name, inputCMND: inputCMND, inputDateCMND: inputDateCMND, inputAddressCMND: inputAddressCMND, birthday: birthday, gioiTinh: gt, nationalText: nationalText, address: address, strBase64Sign: strBase64Sign, serialSim: serialSim, strBase64CMNDTruoc: strBase64CMNDTruoc, strBase64CMNDSau: strBase64CMNDSau, strBase64Avatar: strBase64Avatar, inputAddressPassport: inputAddressPassport, inputSoVisa: inputSoVisa, contractNo: numSO, valuePassport: valuePassport, valueDatePassport: valueDatePassport)
                            ...
                            self.activeItel(phoneNum: phoneNum ?? "", p_TenKH: name, gioiTinh: gt, birthday: birthday, nationalText: nationalText, address: newAddress, serialSim: serialSim, strBase64Sign: strBase64Sign, strBase64CMNDTruoc: strBase64CMNDTruoc, strBase64CMNDSau: strBase64CMNDSau, strBase64Avatar: strBase64Avatar, inputCMND: inputCMND, inputDateCMND: inputDateCMND, inputAddressCMND: inputAddressCMND, inputAddressPassport: inputAddressPassport, inputSoVisa: inputSoVisa, contractNo: numSO, valuePassport: valuePassport, valueDatePassport: valueDatePassport,inputAddressCMNDText: tfCreateAddress.text!)
                        }
                    }
                }
            }
    ...
    func activeViettel(phoneNum: String, p_TenKH: String, gioiTinh: String, birthday: String, nationalText: String, addressVT: String, serialSim: String, strBase64Sign: String, strBase64CMNDTruoc: String, strBase64CMNDSau: String, strBase64Avatar: String, inputCMND: String, inputDateCMND: String, inputAddressCMND: String, inputAddressPassport: String, inputSoVisa: String, contractNo: String, valuePassport: String, valueDatePassport: String) {
            ...
            if(err.count <= 0){
                ...
                if(results.count > 0){
                    ...
                    DispatchQueue.main.asyncAfter(deadline: when) {
                        ...
                        let inputSim = InputSimViettel(p_Duong: "\(addressVT)", p_Phuong: "\(self.wardsButton.text!)", p_Quan: "\(self.districtButton.text!)", p_Tinh: "\(self.cityButton.text!)", p_QuocTich: "\(nationalText)", ChuKy: strBase64Sign, p_SoPhieuYeuCau: "00001", p_TenKH: p_TenKH, p_NgaySinh_KH: birthday, p_GioiTinh_KH: gioiTinh, p_SoCMND_KH: inputCMND, p_NoiCap_CMND_KH: inputAddressCMND, p_NgayCap_CMND_KH: inputDateCMND, p_DiaChi_CMND_KH: inputAddressCMND, p_GoiCuocDK_Line1: "\(self.simActive!.GoiCuoc)", p_SoThueBao_Line1: phoneNum, p_SoSerialSim_Imei_Line1: serialSim, p_Visa: inputSoVisa,strBase64CMNDTruoc:strBase64CMNDTruoc,strBase64CMNDSau:strBase64CMNDSau,strBase64Avatar:strBase64Avatar,simActive:self.simActive!,Nationality:self.selectNational,ProvinceCode:self.selectProvice,DistrictCode:self.selectDistrict,PrecinctCode:self.selectPrecinct,contractNo:"\(contractNo)",LoaiGiayTo:"\(self.typeKichHoat)",Passport:"\(valuePassport)",DayGrantPassport:"\(valueDatePassport)",NoiCapPassport:inputAddressPassport,gtInt:gioiTinh)
    ...
    func activeVinaphone(phoneNum: String, p_TenKH: String, gioiTinh: String, birthday: String, nationalText: String, address: String, serialSim: String, strBase64Sign: String, strBase64CMNDTruoc: String, strBase64CMNDSau: String, strBase64Avatar: String, inputCMND: String, inputDateCMND: String, inputAddressCMND: String, inputAddressPassport: String, inputSoVisa: String, contractNo: String, valuePassport: String, valueDatePassport: String) {
        
        MPOSAPIManager.AutoCreateImageActiveSimVNPhone(p_HopDongSo: "\(self.simActive!.POSSODocNum)", p_ShopDangKy: "\(Cache.user!.ShopName)", p_TenKH: p_TenKH, p_SoDTLienHe: phoneNum, p_NgaySinh: birthday, p_GioiTinh: gioiTinh, p_QuocTich: nationalText, p_SoCMND: inputCMND, p_NgayCap: inputDateCMND, p_NoiCap: inputAddressCMND, p_DiaChiThuongChu: address, p_SoThueBao_Line1: phoneNum, p_Serial_Line1: serialSim, p_GoiCuoc_Line1: "\(self.simActive!.GoiCuoc)", p_ChuKy: strBase64Sign,p_SoNha:"",p_Duong:"",p_To:"",p_Phuong:"",p_Quan:"",p_Tinh:"",p_Email:"", handler: { (imageInfo, err) in
            
            if (err.count<=0){
                print(imageInfo!.ResultImage)
                
                let imagesAll = "\(strBase64CMNDTruoc),\(strBase64CMNDSau),\(imageInfo!.ResultImage),\(strBase64Avatar)"
                let byPassValidateAI = ""
                MPOSAPIManager.ActiveSim(Provider: "\(self.simActive!.Provider)", Phonenumber: phoneNum, Imei: "", SeriSIM: serialSim, IMSI: "", FullName: p_TenKH, Birthday: birthday, Gender: gioiTinh, Address: address, CMND: inputCMND, DateCreateCMND: inputDateCMND, PalaceCreateCMND: inputAddressCMND, Nationality: self.selectNational, ProvinceCode: "\(self.cityButton.text!)", DistrictCode: "\(self.districtButton.text!)", PrecinctCode: "\(self.wardsButton.text!)", contractNo: "\(contractNo)", Note: "", ShopCode: "\(Cache.user!.ShopCode)", UserCode: "\(Cache.user!.UserName)", Images: imagesAll,ID: "\(self.simActive!.ID)",GoiCuoc: "\(self.simActive!.GoiCuoc)",ProductCode: "\(self.simActive!.ProductCode)",LoaiGiayTo:"\(self.typeKichHoat)",Passport:"\(valuePassport)",DayGrantPassport:"\(valueDatePassport)",SOPOS:"\(self.simActive!.POSSODocNum)",NgayHetHanGiayTo:"", TenShop: "\(self.simActive!.TenShop)", DiaChiShop: "\(self.simActive!.DiaChiShop)", TenNhanVien: "\(self.simActive!.TenNhanVien)",NoiCapPassport: inputAddressPassport, SoVisa: inputSoVisa, CustId: "",SSD: "\(self.simActive!.SSD)",IsEsim: 1,Home:"",StreetName:"",StreetBlockName:"",byPassValidateAI: byPassValidateAI, lan: "", ocr_confirm: "", handler: { (status, data) in
    ...
    func activeMobifone(phoneNum: String, p_TenKH: String, birthday: String, gioiTinh: String, quocTich: String, inputCMND: String, inputDateCMND: String, inputAddressCMNDPhieu: String, address: String, serialSim: String, strBase64Sign: String, strBase64CMNDTruoc: String, strBase64CMNDSau: String, strBase64Avatar: String, inputAddressCMND: String, inputAddressPassport: String, inputSoVisa: String, contractNo: String, valuePassport: String, valueDatePassport: String) {
        ...
        MPOSAPIManager.checkGoiCuocVNMobile(phoneNumber: "\(phoneNum)", cmnd: inputCMND, productCode: "\(self.simActive!.ProductCode)", goiCuoc: "\(self.simActive!.GoiCuoc)", userId: "\(Cache.user!.UserName)", maShop: "\(Cache.user!.ShopCode)", nhaMang: simActive!.Provider, handler: { (status, err) in
            if (status){
                ...
                MPOSAPIManager.AutoCreateImageActiveSimMobiPhone(p_HopDongSo: "\(self.simActive!.POSSODocNum)", p_ShopDangKy: "\(Cache.user!.ShopName)", p_TenKH: p_TenKH, p_SoDTLienHe: phoneNum, p_NgaySinh: birthday, p_GioiTinh: gioiTinh, p_QuocGia: quocTich, p_SoCMND: inputCMND, p_NgayCap: inputDateCMND, p_NoiCap: inputAddressCMNDPhieu, p_DiaChiThuongChu: address, p_SoThueBao_Line1: phoneNum, p_Serial_Line1: serialSim, p_GoiCuoc_Line1: "\(self.simActive!.GoiCuoc)", p_ChuKy: strBase64Sign, handler: { (imageInfo, err) in
                    if (err.count<=0){
    ...
    func activeVietnammobile(phoneNum: String, p_TenKH: String, inputCMND: String, inputDateCMND: String, inputAddressCMND: String, birthday: String, gioiTinh: String, nationalText: String, address: String, strBase64Sign: String, serialSim: String, strBase64CMNDTruoc: String, strBase64CMNDSau: String, strBase64Avatar: String, inputAddressPassport: String, inputSoVisa: String, contractNo: String, valuePassport: String, valueDatePassport: String) {
        
        MPOSAPIManager.checkGoiCuocVNMobile(phoneNumber: "\(phoneNum)", cmnd: inputCMND, productCode: "\(self.simActive!.ProductCode)", goiCuoc: "\(self.simActive!.GoiCuoc)", userId: "\(Cache.user!.UserName)", maShop: "\(Cache.user!.ShopCode)", nhaMang: simActive!.Provider, handler: { (status, err) in
            if (status){
                MPOSAPIManager.AutoCreateImageActiveSimVietnamobile(p_HopDongSo:"\(self.simActive!.POSSODocNum)", p_TenKH: p_TenKH, p_CMND_KH: inputCMND, p_NgayCapCMND_KH: inputDateCMND, p_NoiCapCMND_KH: inputAddressCMND, p_NgaySinh_KH: birthday, p_GioiTinh_KH: gioiTinh, p_QuocGia_KH: "\(nationalText)", p_NoiThuongTru_KH: "\(address)", p_ChuKy: strBase64Sign, p_SoThueBao_Line1: phoneNum, p_SoICCID_Line1: serialSim, handler: { (imageInfo, err) in
                    
                    if (err.count<=0){
                        let imagesAll = "\(strBase64CMNDTruoc),\(strBase64CMNDSau),\(imageInfo!.ResultImage),\(strBase64Avatar)"
                        
                        MPOSAPIManager.ActiveSim(Provider: "\(self.simActive!.Provider)", Phonenumber: phoneNum, Imei: "", SeriSIM: serialSim, IMSI: "", FullName: p_TenKH, Birthday: birthday, Gender: gioiTinh, Address: address, CMND: inputCMND, DateCreateCMND: inputDateCMND, PalaceCreateCMND: inputAddressCMND, Nationality: nationalText, ProvinceCode: self.selectProvice, DistrictCode: self.selectDistrict, PrecinctCode: self.selectPrecinct, contractNo: "\(contractNo)", Note: "", ShopCode: "\(Cache.user!.ShopCode)", UserCode: "\(Cache.user!.UserName)", Images: imagesAll,ID: "\(self.simActive!.ID)",GoiCuoc: "\(self.simActive!.GoiCuoc)",ProductCode: "\(self.simActive!.ProductCode)",LoaiGiayTo:"\(self.typeKichHoat)",Passport:"\(valuePassport)",DayGrantPassport:"\(valueDatePassport)",SOPOS:"\(self.simActive!.POSSODocNum)",NgayHetHanGiayTo:"", TenShop: "\(self.simActive!.TenShop)", DiaChiShop: "\(self.simActive!.DiaChiShop)", TenNhanVien: "\(self.simActive!.TenNhanVien)",NoiCapPassport: inputAddressPassport, SoVisa: inputSoVisa, CustId: "",SSD: "\(self.simActive!.SSD)",IsEsim: 1,Home:"",StreetName:"",StreetBlockName:"",byPassValidateAI: "", lan: "", ocr_confirm: "", handler: { (status, data) in
    ...
    func activeItel(phoneNum: String, p_TenKH: String, gioiTinh: String, birthday: String, nationalText: String, address: String, serialSim: String, strBase64Sign: String, strBase64CMNDTruoc: String, strBase64CMNDSau: String, strBase64Avatar: String, inputCMND: String, inputDateCMND: String, inputAddressCMND: String, inputAddressPassport: String, inputSoVisa: String, contractNo: String, valuePassport: String, valueDatePassport: String, inputAddressCMNDText:String) {
        
        MPOSAPIManager.CustomerResult_HDItel(p_HopDongSo: contractNo, p_TenKH: p_TenKH, p_CMND_KH: inputCMND, p_NgayCapCMND_KH: inputDateCMND, p_NoiCapCMND_KH: inputAddressCMND, p_NgaySinh_KH: birthday, p_GioiTinh_KH: gioiTinh, p_QuocGia_KH: nationalText, p_NoiThuongTru_KH: address, p_ChuKy: strBase64Sign, p_SoThueBao_Line1: phoneNum, p_SoICCID_Line1: serialSim, p_GoiCuoc_Line1: "\(self.simActive?.GoiCuoc ?? "")", p_SoDTLienHe: phoneNum) { (imageInfo, err) in
            
            if err.count <= 0 {
                debugPrint(imageInfo!.ResultImage)
                ...
                MPOSAPIManager.ActiveSim(Provider: "\(self.simActive!.Provider)", Phonenumber: phoneNum, Imei: "", SeriSIM: serialSim, IMSI: "", FullName: p_TenKH, Birthday: birthday, Gender: gioiTinh, Address: address, CMND: inputCMND, DateCreateCMND: inputDateCMND, PalaceCreateCMND: inputAddressCMND, Nationality: self.selectNational, ProvinceCode: self.selectProvice, DistrictCode: self.selectDistrict, PrecinctCode: self.selectPrecinct, contractNo: "\(contractNo)", Note: "", ShopCode: "\(Cache.user!.ShopCode)", UserCode: "\(Cache.user!.UserName)", Images: imagesAll,ID: "\(self.simActive!.ID)",GoiCuoc: "\(self.simActive!.GoiCuoc)",ProductCode: "\(self.simActive!.ProductCode)",LoaiGiayTo:"\(self.typeKichHoat)",Passport:"\(valuePassport)",DayGrantPassport:"\(valueDatePassport)",SOPOS:"\(self.simActive!.POSSODocNum)",NgayHetHanGiayTo:"", TenShop: "\(self.simActive!.TenShop)", DiaChiShop: "\(self.simActive!.DiaChiShop)", TenNhanVien: "\(self.simActive!.TenNhanVien)",NoiCapPassport: inputAddressPassport, SoVisa: inputSoVisa, CustId: "",SSD: "\(self.simActive!.SSD)",IsEsim: 1,Home:"",StreetName:"",StreetBlockName:"",byPassValidateAI: byPassValidateAI, lan: "", ocr_confirm: "", handler: { (status, data) in
                    ...
                    DispatchQueue.main.asyncAfter(deadline: when) {
                        if (status){
                            ...
                            alert.addAction(UIAlertAction(title: "OK", style: .default, handler: {
                                (alert: UIAlertAction!) -> Void in
                                self.genQRCodeItel(serial: serialSim, phoneNum: phoneNum)
                            }))
                            self.present(alert, animated: true, completion: nil)
                        }else{
                            var data1 = ""
...
//                                    self.dismiss(animated: true, completion: nil)
    ...
    func getSeriSim(){
        let newViewController = LoadingViewController()
        newViewController.content = "Đang lấy Seri Sim..."
        newViewController.modalPresentationStyle = UIModalPresentationStyle.overCurrentContext
        newViewController.modalTransitionStyle = UIModalTransitionStyle.crossDissolve
        self.navigationController?.present(newViewController, animated: true, completion: nil)
        let nc = NotificationCenter.default
        
        MPOSAPIManager.sp_mpos_FRT_SP_Esim_getSeri(SDT:
        self.simActive!.Phonenumber,ItemCode: "\(self.simActive!.GoiCuoc)",SoMpos: "\(self.simActive!.POSSODocNum)") { (results, err) in
            let when = DispatchTime.now() + 0.5
            DispatchQueue.main.asyncAfter(deadline: when) {
                nc.post(name: Notification.Name("dismissLoading"), object: nil)
                if (err.count <= 0){
                    if(results.count > 0){
                        if results[0].seri.isEmpty {
                            let popup = PopupDialog(title: "Thông báo", message: "\(results[0].p_messagess)", buttonAlignment: .horizontal, transitionStyle: .zoomIn, tapGestureDismissal: false, panGestureDismissal: false) {
                                
                            }
                            let buttonOne = CancelButton(title: "Trở về") {
                                self.navigationController?.popViewController(animated: true)
                            }
                            let buttonTwo = DefaultButton(title: "Tải lại") {
                                self.getSeriSim()
                            }
                            popup.addButtons([buttonTwo,buttonOne])
                            self.present(popup, animated: true, completion: nil)
                        } else {
                            self.seriSim = results[0].seri
                        }
                        
                    }else{
                        let popup = PopupDialog(title: "Thông báo", message: "Data Seri Sim trả rỗng !!!", buttonAlignment: .horizontal, transitionStyle: .zoomIn, tapGestureDismissal: false, panGestureDismissal: false) {
    ...
    func getQRcode(){
        ...
        self.simActive!.Phonenumber,SOMPOS: "\(self.simActive!.POSSODocNum)",SeriSim: "\(self.seriSim)") { (results, err) in
            let when = DispatchTime.now() + 0.5
            DispatchQueue.main.asyncAfter(deadline: when) {
                nc.post(name: Notification.Name("dismissLoading"), object: nil)
                if (err.count <= 0){
                    results[0].sdt = self.simActive!.Phonenumber
    ...
    func genQRCodeItel(serial: String, phoneNum: String) {
        WaitingNetworkResponseAlert.PresentWaitingAlertWithContent(parentVC: self, content: "Đang gen qrcode...") {
                ...
                WaitingNetworkResponseAlert.DismissWaitingAlert {
                    if err.count <= 0 {
                        if errMsg.count <= 0 {
                            ...
                            let popup = PopupDialog(title: "Thông báo", message: "\(errMsg)\n Số thuê bao: \(phoneNum) \n Seri: \(serial)", buttonAlignment: .horizontal, transitionStyle: .zoomIn, tapGestureDismissal: false, panGestureDismissal: false) {
                            }
                            let buttonOne = CancelButton(title: "OK") {
                            }
                            popup.addButtons([buttonOne])
...
extension DetailEsimViewController:  UIImagePickerControllerDelegate, UINavigationControllerDelegate{
...
extension DetailEsimViewController: CalendarViewController1Delegate {
