...
protocol NCCThuHoViewControllerDelegate: NSObjectProtocol {
    func returnProvider(item:ThuHoProvider,ind:Int)
}

class NCCThuHoViewController: UIViewController, UITableViewDataSource, UITableViewDelegate{
    
    var delegate:NCCThuHoViewControllerDelegate?
    var barClose : UIBarButtonItem!
    
    var tableView: UITableView = UITableView()
    var items: [ThuHoProvider] = []
    let searchController = UISearchController(searchResultsController: nil)
    var filteredCandies = [ThuHoProvider]()
    ...
    override func viewDidLoad() {
        ...
        tableView.register(ItemThuHoProviderCell.self, forCellReuseIdentifier: "ItemThuHoProviderCell")
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell:ItemThuHoProviderCell = tableView.dequeueReusableCell(withIdentifier: "ItemThuHoProviderCell", for: indexPath) as! ItemThuHoProviderCell
        var candy: ThuHoProvider! = nil
        if isFiltering() {
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath)
    {
        ...
        delegate?.returnProvider(item:item,ind:ind)
    }
    
    // MARK: - Private instance methods
    
    func filterContentForSearchText(_ searchText: String, scope: String = "") {
        filteredCandies = items.filter({( candy : ThuHoProvider) -> Bool in
            let doesCategoryMatch = (scope == "") || (candy.PaymentBillServiceName == scope)
            
            if searchBarIsEmpty() {
                return doesCategoryMatch
            } else {
                return doesCategoryMatch && candy.PaymentBillProviderName.lowercased().contains(searchText.lowercased())
            }
        })
...
extension NCCThuHoViewController: UISearchBarDelegate {
...
extension NCCThuHoViewController: UISearchResultsUpdating {
    // MARK: - UISearchResultsUpdating Delegate
    func updateSearchResults(for searchController: UISearchController) {
        filterContentForSearchText(searchController.searchBar.text!, scope: "")
    }
}
class ItemThuHoProviderCell: UITableViewCell {
    var type: UILabel!
    var name: UILabel!
