...
class DetailCreateCustomer: BaseController, RadioCustomDelegate, ImageFrameCustomDelegate {
	...
	private func editCustomerCoreInnstallment() {
		let firstName = tfHo.text ?? ""
		let middleName = tfTenDem.text ?? ""
		let lastName = tfName.text ?? ""
	...
	@objc func tapFromAddress() {
		...
		vc.onSelectSuccess = { [weak self] fullAddress in
			...
			case "edit":
				vc.flow = self.flow
				self.tfAddressDefault.text =
					CoreInstallMentData.shared.editItemDiaChiTamTru["fullAddress"] as? String
			case "create":
				vc.flow = self.flow
				self.tfAddress.text =
					CoreInstallMentData.shared.itemDiaChiTamTru["fullAddress"] as? String
			default:
				return
			}
	...
	private func setupUI() {
		...
		//        tfIssueBy.withImage(direction: .left, image: UIImage(named: "ic_map2")!)
		switch flow {
		case "create":
		...
		case "edit":
			...
			tfHo.text = CoreInstallMentData.shared.editFirstName
			tfTenDem.text = CoreInstallMentData.shared.editMiddleName
			tfName.text = CoreInstallMentData.shared.editLastName
			...
			tfAddress.text = CoreInstallMentData.shared.editItemDiaChiTT["fullAddress"] as? String

			//                onClickRadio(radioView: UIView(), tag: sexTag)
			tfIDCard.text = CoreInstallMentData.shared.editIdCard
			tfIdCardIssuedDate.text = CoreInstallMentData.shared.editIdCardIssuedExpiration
	...
	@IBAction func onClickContinue(_ sender: Any) {
		...
		CoreInstallMentData.shared.customerInforFromDetechCMND?.idCardIssuedDate = tfIdCardIssuedDate.text ?? ""
	...
	private func validateInputs() -> Bool {
		...
		guard let name = tfAddress.text, !name.isEmpty else {
			showAlertOneButton(
				title: "Thông báo", with: "Vui lòng chọn địa chỉ thườg trú", titleButton: "OK")
			return false
		}
		...
		guard let name = tfIssueBy.text, !name.isEmpty else {
			showAlertOneButton(title: "Thông báo", with: "Vui chọn ngày nơi cấp", titleButton: "OK")
			return false
		}
