...
class SearchCustomerController: UIViewController {
        //MARK: - Variable
...
//        search(shouldShow: false)
        ...
        searchTextField.text = "301569972"
...
//    private func searchInfoCustomer(keyword: String,skipCount:Int,maxResultCount:Int) {
//        Provider.shared.createCustomerAPIService.searchInfoCustomer(keyword: keyword, skipCount: skipCount, maxResultCount: maxResultCount, success: { [weak self] result in
//            guard let self = self, let response = result?.items else { return }
//            print("**1***")
//            print(result)
...
//            searchInfoCustomer(keyword: searchBar.text ?? "",skipCount: 0, maxResultCount: 10)
            searchInfoByIDCard(keyword: searchBar.text ?? "")
		}
		tableView.reloadData()
...
//        vc.idCard = filterList[indexPath.row].idCard ?? ""
    ...
    func tableView(_ tableView: UITableView, trailingSwipeActionsConfigurationForRowAt indexPath: IndexPath) -> UISwipeActionsConfiguration? {
        ...
        let delete = UIContextualAction(style: .destructive, title: "Xóa") { (action, view, completionHandler) in
            ...
            let refreshAlert = UIAlertController(title: "Thông báo", message: "Bạn có đồng ý hủy số phiếu \(self.filterList[indexPath.row].id ?? 0) ?", preferredStyle: .alert)
