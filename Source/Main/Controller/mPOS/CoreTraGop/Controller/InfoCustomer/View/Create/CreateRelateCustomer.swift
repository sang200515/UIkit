...
class CreateRelateCustomer: BaseController {
    ...
    var titleNav = "Thông tin người liên hệ"
	//MARK: - Properties
...
//		tableView.reloadData()
	...
	private func actionEdit() {
		if flow == "edit" {
			...
			let liistRefPersons = CoreInstallMentData.shared.editItemListNguoiLienHe
			...
			Provider.shared.createCustomerAPIService.editCustomerCoreInstallment(
				...
				failure: { [weak self] error in
					guard let self = self else { return }
					self.showAlertOneButton(
                        title: "Thông báo", with: "", titleButton: "OK")
				})
		} else {
	...
	@IBAction func onClickContinue(_ sender: Any) {
		...
		if flow == "create" {
            let vc = CreateRelateProfile()
            CoreInstallMentData.shared.isCreateNguoiLienHe = true
            vc.flow = flow
            self.navigationController?.pushViewController(vc, animated: true)
		} else {
			actionEdit()
