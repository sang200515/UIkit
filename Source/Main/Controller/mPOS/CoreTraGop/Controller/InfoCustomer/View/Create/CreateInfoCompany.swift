...
class CreateInfoCompany: BaseController, UITextFieldDelegate {
	...
	//MARK: - Properties
	var flow: String = ""
var titleNav = "Thông tin công ty"
	...
	override func viewDidLoad() {
		...
		setupUITextFieldUI()
		title = "thông tin công ty khách hàng"
		createFlow()
		editFlow()
	}
	...
	private func createFlow() {
		if flow == "create" {
            if CoreInstallMentData.shared.isCreatedCompany {
                currentString = ""
                tfName.text = CoreInstallMentData.shared.itemDiaChiCty["companyName"] as? String ?? ""
                tfPosition.text = CoreInstallMentData.shared.itemDiaChiCty["position"] as? String ?? ""
                tfAddress.text = "\(CoreInstallMentData.shared.itemDiaChiCtyDetail["fullAddress"] ?? "")"
                tfYear.text = "\(CoreInstallMentData.shared.itemDiaChiCty["workYear"] ?? "")"
                tfMonth.text =  "\(CoreInstallMentData.shared.itemDiaChiCty["workMonth"] ?? "")"
                let income:Int = Int(CoreInstallMentData.shared.itemDiaChiCty["income"] as? Float ?? 0)
                    tfIncome.text = "\(Common.convertCurrencyV2(value: income))"
                tfPhone.text = CoreInstallMentData.shared.itemDiaChiCty["companyPhone"] as? String ?? ""
	...
	private func editFlow() {
		if flow == "edit" {
            currentString = "\(CoreInstallMentData.shared.editInCome)"
			...
			tfAddress.text = CoreInstallMentData.shared.editDiachiCty
			tfYear.text = "\(Int(itemDetail?.company?.workYear ?? 0))"
			tfIncome.text = "\(Common.convertCurrencyV2(value: CoreInstallMentData.shared.editInCome))"
	...
	private func setupUITextFieldUI() {
		...
		tfIncome.clearButtonMode = .whileEditing
	...
	private func validateInputs() -> Bool {
        ...
        if (Int(tfYear.text ?? "0") ?? 0) > 60 {
            showAlertOneButton(
                title: "Thông báo", with: "Bạn vui lòng nhập lại số năm làm việc,tối đa 60 năm ",
                titleButton: "OK")
            return false
        }
		if (Int(tfMonth.text ?? "0") ?? 0) > 12 {
			showAlertOneButton(
				title: "Thông báo", with: "Bạn vui lòng nhập lại số tháng làm việc tối đa 12 tháng ",
				titleButton: "OK")
			return false
		}
	...
	@IBAction func onClickContinue(_ sender: Any) {
		...
		case "create":
            ...
            CoreInstallMentData.shared.isCreatedCompany = true
            CoreInstallMentData.shared.isCreatedCompanyAddress = true
