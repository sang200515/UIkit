...
class CreateRelateProfile: BaseController, RadioCustomDelegate, ImageFrameCustomDelegate {
    ...
    @IBOutlet weak var tableViewSHKHeight: NSLayoutConstraint!
	...
	//
	//	}
    var titleNav = "Giấy tờ liên quan"
	var flow: String = ""
    let tableViewCellHeight: CGFloat = 170
	//MARK: - Variable
...
//    @IBOutlet weak var heightTableView: NSLayoutConstraint!
    ...
    private func autoFillParam(){
        if flow == "create"{

            if CoreInstallMentData.shared.isCreateRelateProfile {
                tfSoGPLX.text = CoreInstallMentData.shared.itemGiayTo["drivingLicNum"] as? String ?? ""
                tfNgayCap.text = CoreInstallMentData.shared.itemGiayTo["drivingLicNumDate"] as? String ?? ""
                tfNoiCap.text = CoreInstallMentData.shared.itemGiayTo["drivingLicNumBy"] as? String ?? ""
                soSHKTxt.text = CoreInstallMentData.shared.itemGiayTo["houseBookNum"] as? String ?? ""
                hoTenChuHoTxt.text = CoreInstallMentData.shared.itemGiayTo["houseBookName"] as? String ?? ""
                diaChiThuongTruTxt.text = CoreInstallMentData.shared.itemGiayTo["houseBookAddress"] as? String ?? ""

            }
//            if CoreInstallMentData.shared.pathGPLXfront["fileType"] as? String != "" {
//                let urlImage:String = CoreInstallMentData.shared.pathGPLXfront["urlImage"] as? String ?? ""
...
//                    }
//            }
//            gplxRadio.setSelect(isSelect: paper)
//            onClickRadio(radioView: gplxRadio, tag: 0)
	...
	private func setupUI() {
        ...
        tfSoGPLX.clearButtonMode = .whileEditing
        ...
        hoTenChuHoTxt.clearButtonMode = .whileEditing
        soSHKTxt.clearButtonMode = .whileEditing
...
//                        shkRadio.isUserInteractionEnabled = false
//                        gplxRadio.isUserInteractionEnabled =  true
                        self.setImage(imageFrameCustom: self.gplxFront)
...
//                shkRadio.isUserInteractionEnabled = true
//                gplxRadio.isUserInteractionEnabled = false
	...
	// MARK: - API
	// MARK: - Helpers
	private func addItemGiayTo() -> [String: Any] {
		var detail = [String: Any]()
	...
	private func validateInputs() -> Bool {
        CoreInstallMentData.shared.itemGiayTo["drivingLicNum"] = tfSoGPLX.text ?? ""
        CoreInstallMentData.shared.itemGiayTo["drivingLicNumDate"] = tfNgayCap.text ?? ""
        CoreInstallMentData.shared.itemGiayTo["drivingLicNumBy"] = tfNoiCap.text ?? ""
        CoreInstallMentData.shared.itemGiayTo["houseBookNum"] = soSHKTxt.text ?? ""
        CoreInstallMentData.shared.itemGiayTo["houseBookName"] = hoTenChuHoTxt.text ?? ""
        CoreInstallMentData.shared.itemGiayTo["houseBookAddress"] = diaChiThuongTruTxt.text ?? ""
        CoreInstallMentData.shared.isCreateRelateProfile = true
        if paper == "" {
            self.showAlert("Bạn vui lòng chọn loại giấy tờ")
            return false
        }
		if paper == "FB" {
            ...
            if flow == "edit" {
                if CoreInstallMentData.shared.editListSHKImage.count < 3 {
                    self.showAlert("Bạn vui lòng chụp ít nhất 3 ảnh Sổ hộ khẩu")
                    return false
                }
            }else{
                if CoreInstallMentData.shared.listSHKImage.count < 4 {
                    if CoreInstallMentData.shared.listSHKImage[0]["fileType"] as? String == "" {
                        self.showAlert("Bạn vui lòng chụp ít nhất 3 ảnh Sổ hộ khẩu")
                        return false
                    }
                    if CoreInstallMentData.shared.listSHKImage[1]["fileType"] as? String == "" {
                        self.showAlert("Bạn vui lòng chụp ít nhất 3 ảnh Sổ hộ khẩu")
                        return false
                    }
                    if CoreInstallMentData.shared.listSHKImage[2]["fileType"] as? String == "" {
                        self.showAlert("Bạn vui lòng chụp ít nhất 3 ảnh Sổ hộ khẩu")
                        return false
                    }
	...
	@IBAction func addNewImgSHK(_ sender: Any) {
		...
		if flow == "create" {
            let lastIndex = CoreInstallMentData.shared.listSHKImage.count - 1
            if CoreInstallMentData.shared.listSHKImage[lastIndex]["urlImage"] as? String == ""{
                return
            }else {
                Toast.init(text: "Đã thêm thành công trang \( CoreInstallMentData.shared.listSHKImage.count)").show( )
                CoreInstallMentData.shared.listSHKImage.append(addItemUploadSHK())
                tableVewSoHoKhau.beginUpdates()
                self.tableVewSoHoKhau.insertRows(at: [IndexPath(row: CoreInstallMentData.shared.listSHKImage.count-1, section: 0)], with: .automatic)
                calculateTableviewHeight()
                scrollToBottom()
                self.tableVewSoHoKhau.endUpdates()
            }


		} else {
            let lastIndex = CoreInstallMentData.shared.editListSHKImage.count - 1
            if CoreInstallMentData.shared.editListSHKImage.count > 0 {
                if CoreInstallMentData.shared.editListSHKImage[lastIndex]["urlImage"] as? String == ""{
                    return
                }else{
   ...
   private func scrollToBottom(){
       ...
       let offset = self.scrollView.contentSize.height - self.scrollView.visibleSize.height
	...
	private func createCustomerCoreInnstallment() {
		...
		guard validateInputs() else { return }
		CoreInstallMentData.shared.itemGiayTo = addItemGiayTo()
		...
		let idCardIssuedDate = CoreInstallMentData.shared.idCardIssuedDate  ?? ""
		let idCardIssuedExpiration = CoreInstallMentData.shared.idCardIssuedExpiration
		let phone = CoreInstallMentData.shared.phone
		let relatedDocument = CoreInstallMentData.shared.itemGiayTo
    ...
    func calculateTableviewHeight() {
        if flow == "create" {
            tableViewSHKHeight.constant = CGFloat(CoreInstallMentData.shared.listSHKImage.count) * tableViewCellHeight
        } else {
            tableViewSHKHeight.constant = CGFloat(CoreInstallMentData.shared.editListSHKImage.count) * tableViewCellHeight
        }
