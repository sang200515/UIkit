//
//  LSViettelPaySOMViewControllerV2.swift
//  fptshop
//
...
class LSViettelPaySOMViewControllerV2: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        btnCheck.backgroundColor = UIColor(netHex: 0x00955E)
        btnCheck.layer.cornerRadius = 8
        btnCheck.addTarget(self, action: #selector(loadDataHistory), for: .touchUpInside)
        ...
        tableView = UITableView(frame: CGRect(x: 0, y: btnCheck.frame.origin.y + btnCheck.frame.height + Common.Size(s: 15), width: self.view.frame.width, height: tableViewHeight - btnCheck.frame.origin.y - btnCheck.frame.height - Common.Size(s: 20)))
        self.view.addSubview(tableView)
        tableView.delegate = self
        tableView.dataSource = self
        tableView.separatorStyle = .none
        tableView.register(ViettelPaySOMHistoryCell.self, forCellReuseIdentifier: "viettelPaySOMHistoryCell")
    ...
    @objc func loadDataHistory() {
        
        WaitingNetworkResponseAlert.PresentWaitingAlert(parentVC: self) {
            CRMAPIManager.ViettelPay_SOM_GetHistory(fromDate: "\((self.lbFromDateText.text ?? "").trim())", toDate: "\((self.lbToDateText.text ?? "").trim())", parentCategoryIds: "\(self.cateID)") { (rs, err) in
...
extension LSViettelPaySOMViewControllerV2: UITableViewDelegate, UITableViewDataSource {
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        return list.count
    }
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell: ViettelPaySOMHistoryCell = tableView.dequeueReusableCell(withIdentifier: "viettelPaySOMHistoryCell", for: indexPath) as! ViettelPaySOMHistoryCell
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        let item = list[indexPath.row]
        let vc = ResultRechargeHistoryViettelPay()
        vc.itemViettelPayHistory = item
        vc.orderID = item.orderId
        self.navigationController?.pushViewController(vc, animated: true)
    }
}

class ViettelPaySOMHistoryCell: UITableViewCell {
    
    var item: ViettelPaySOMHistory?
    ...
    func setUpCell() {
        ...
        lbSdtKHText.textColor = UIColor(netHex: 0x4fa845)
        ...
        if item?.orderStatus == 2 {
            lbTrangThaiText.textColor = UIColor(netHex: 0x4fa845)
