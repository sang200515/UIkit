//
//  ReceiveMoneyViettelPayViewController.swift
//  fptshop
//
...
import Foundation
import UIKit
class ReceiveMoneyViettelPayViewController: UIViewController ,UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout{
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        if(item.type == "1"){
            let vc = CreateReceiveMoneyViettelPayViewController()
            self.navigationController?.pushViewController(vc, animated: true)
        }else if(item.type == "2"){
            let vc = HistoryReceiveMoneyViettelPayViewController()
            self.navigationController?.pushViewController(vc, animated: true)
        }
    }
