//
//  TransferMoneyMenuViettelPayViewController.swift
//  fptshop
//
...
import Foundation
import UIKit
class TransferMoneyMenuViettelPayViewController: UIViewController ,UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    ...
    override func viewDidLoad() {
        ...
        btBackIcon.addTarget(self, action: #selector(ReceiveMoneyViettelPayViewController.actionBack), for: UIControl.Event.touchUpInside)
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        if(item.type == "1"){
            let vc = TransferMoneyViettelPayViewController()
            self.navigationController?.pushViewController(vc, animated: true)
        }else if(item.type == "2"){
            let vc = HistoryTransferMoneyViettelPayViewController()
            self.navigationController?.pushViewController(vc, animated: true)
        }
    }
