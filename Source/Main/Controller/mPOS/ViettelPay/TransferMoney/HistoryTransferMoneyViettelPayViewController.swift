//
//  HistoryTransferMoneyViettelPayViewController.swift
//  fptshop
//
...
import PopupDialog
class HistoryTransferMoneyViettelPayViewController:  UIViewController,UITableViewDataSource, UITableViewDelegate {
    var tableView: UITableView  =   UITableView()
    var items: [TransferHeaders] = []
    var listTemp:[TransferHeaders] = []
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        self.title = "Lịch sử GD chuyển tiền"
        ...
        btLeftIcon.addTarget(self, action: #selector(HistoryTransferMoneyViettelPayViewController.backButton), for: UIControl.Event.touchUpInside)
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        MPOSAPIManager.getMakeTransferHeaders(handler: { (results, err) in
            if(results.count > 0){
    ...
    func setupUI(list: [TransferHeaders]){
        tableView.frame = CGRect(x: 0, y:0, width: loadingView.frame.size.width, height: self.view.frame.size.height )
        //- (UIApplication.shared.statusBarFrame.height + Cache.heightNav)
        tableView.dataSource = self
        tableView.delegate = self
        tableView.register(ItemTransferMoneyViettelPayTableViewCell.self, forCellReuseIdentifier: "ItemTransferMoneyViettelPayTableViewCell")
...
//        refreshControl.attributedTitle = NSAttributedString(string: "Đang load dữ liệu ...", attributes: secondAttributes)
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath)
    {
        ...
        let item:TransferHeaders = items[indexPath.row]
        ...
        MPOSAPIManager.GetMakeTransferDetails(docentry: "\(item.docentry)") { (results, err) in
            let when = DispatchTime.now() + 0.5
            DispatchQueue.main.asyncAfter(deadline: when) {
                nc.post(name: Notification.Name("dismissLoading"), object: nil)
                
                if(err.count <= 0){
                    results!.trangthai = item.TrangThai
                    results!.NgayGiaoDich = item.NgayGiaoDich
                    let newViewController = ResultHistoryTransViettelPayViewController()
                    newViewController.transferDetail = results
    ...
    {
        let cell = ItemTransferMoneyViettelPayTableViewCell(style: UITableViewCell.CellStyle.default, reuseIdentifier: "ItemTransferMoneyViettelPayTableViewCell")
        let item:TransferHeaders = items[indexPath.row]
        cell.setup(so: item)
...
extension HistoryTransferMoneyViettelPayViewController: UISearchBarDelegate {
...
extension HistoryTransferMoneyViettelPayViewController: UISearchResultsUpdating {
...
class ItemTransferMoneyViettelPayTableViewCell: UITableViewCell {
    ...
    var so1:TransferHeaders?
    func setup(so:TransferHeaders){
        so1 = so
//
...
//
        ...
        tenKH.text = "Họ tên KH gửi: \(so.sender_name)"
