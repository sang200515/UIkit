...
class RaPCViewController: UIViewController {
    ...
    var filterList:[DataRaPC] = []
    var listDataPc:[DataRaPC] = []
...
//        //replace
//        loadSearchItems(shopCode: Cache.user!.ShopCode, status: "", from: "1/10/2021", to: "01/12/2021", itemcode: "")
    }
...
//                loadSearchItems(shopCode: Cache.user!.ShopCode, status: "", from: "1/10/2021", to: "01/12/2021", itemcode: "")
    ...
    func loadSearchItems(shopCode:String,status:String,from:String,to:String,itemcode:String) {
        ...
        Provider.shared.raPCAPService.getListPC(shopCode: shopCode, status: status, from: from, to: to, itemCode: itemcode,requestType: "2", success: { [weak self] result in
    ...
    func configureUI() {
        ...
        navigationItem.title = "DANH SÁCH RÃ PC"
...
extension RaPCViewController: UITableViewDelegate,UITableViewDataSource  {
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        let vc = RaPCDetailViewController()
              ...
              completionHandler(true)
                ...
                refreshAlert.addAction(UIAlertAction(title: "Đồng ý", style: .destructive, handler: { (action: UIAlertAction!) in
                      print("Handle Ok logic here")
                        ...
                        ProgressView.shared.show()
                        Provider.shared.raPCAPService.updateStatePCBuild(doc_request: self.filterList[indexPath.row].docEntry_Request, doc_header: self.filterList[indexPath.row].docEntry, status: "4", update_by_code: Cache.user!.UserName, update_by_name: Cache.user!.EmployeeName, success: { [weak self] result in
                            ProgressView.shared.hide()
                            guard let self = self,let response = result else {return}
                            if response.message?.message_Code == 200 {
