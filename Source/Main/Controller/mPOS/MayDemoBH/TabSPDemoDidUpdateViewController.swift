//
//  TabSPDemoDidUpdateViewController.swift
//  fptshop
//
...
class TabSPDemoDidUpdateViewController: UIViewController {
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        MPOSAPIManager.Products_Demo_Warranty_ListProduct(status: "C") { (rs, err) in
            if err.count <= 0 {
                if rs.count > 0 {
                    self.listSp = rs
    ...
    func setUpView() {
        ...
        let tableViewHeight:CGFloat = self.view.frame.height - Common.Size(s: 50) - (self.parentNavigationController?.navigationBar.frame.height ?? 0 + UIApplication.shared.statusBarFrame.height)
...
extension TabSPDemoDidUpdateViewController: UITableViewDelegate, UITableViewDataSource {
