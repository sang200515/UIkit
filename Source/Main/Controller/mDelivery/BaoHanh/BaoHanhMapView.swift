...
class BaoHanhMapView: UIView {
    ...
    override init(frame: CGRect) {
        ...
        //companyButton.theme.fontColor = UIColor.red
        ...
        company2Button.placeholder = "điểm kết thúc"
        ...
        txtXemBanDo = UILabel(frame: CGRect(x: UIScreen.main.bounds.size.width - sizeNameString.width, y: company2Button.frame.origin.y +  company2Button.frame.size.height + Common.Size(s:10), width: UIScreen.main.bounds.size.width, height: sizeNameString.height))
        txtXemBanDo.textAlignment = .left
        txtXemBanDo.textColor = UIColor(netHex:0x095b8e)
        txtXemBanDo.font = UIFont.italicSystemFont(ofSize: Common.Size(s:13))
        txtXemBanDo.text = "Xem bản đồ"
        
        mapKit = MKMapView(frame: CGRect(x: 20, y: txtXemBanDo.frame.origin.y +  txtXemBanDo.frame.size.height + Common.Size(s:10), width: UIScreen.main.bounds.size.width - 40, height: 200))
        viewGoogleMap = UIView(frame: CGRect(x: 20, y: txtXemBanDo.frame.origin.y +  txtXemBanDo.frame.size.height + Common.Size(s:10), width: UIScreen.main.bounds.size.width - 40, height: 200))
        viewGoogleMap.backgroundColor = UIColor.black
        
        mCamera = GMSCameraPosition.camera(withLatitude: -33.86, longitude: 151.20, zoom: 6.0)
        mapView = GMSMapView.map(withFrame: CGRect(x: 20, y: txtXemBanDo.frame.origin.y +  txtXemBanDo.frame.size.height + Common.Size(s:10), width: UIScreen.main.bounds.size.width - 40, height: 200), camera: mCamera)
        ...
        viewBottom = UIView(frame: CGRect(x: 0,y: mapKit.frame.origin.y + mapKit.frame.size.height + Common.Size(s:10) ,width:UIScreen.main.bounds.size.width , height: Common.Size(s:30)  + sizeNameString.height * 14 + Common.Size(s:40)))
        ...
        btnDoiLoTrinh = UIButton(frame: CGRect(x:(UIScreen.main.bounds.size.width - txtLoTrinh.frame.size.width / 2) / 2, y: Common.Size(s:10), width: txtLoTrinh.frame.size.width / 2  , height: Common.Size(s:30)));
        btnDoiLoTrinh.backgroundColor = UIColor(netHex:0xEF4A40)
        //btnHoanTat.layer.cornerRadius = 20
        btnDoiLoTrinh.layer.borderWidth = 0.5
        btnDoiLoTrinh.layer.borderColor = UIColor.white.cgColor
        btnDoiLoTrinh.layer.cornerRadius = 5.0
        btnDoiLoTrinh.setTitle("Chỉ đường",for: .normal)
        btnDoiLoTrinh.setTitleColor(UIColor.white, for: .normal)
        
        
        
        
        txtThongTinBaoHanh = UILabel(frame: CGRect(x: 20, y: btnDoiLoTrinh.frame.origin.y , width: UIScreen.main.bounds.size.width, height: sizeNameString.height))
        ...
        txtTongPHBNhan = UILabel(frame: CGRect(x: 20, y: viewLine.frame.origin.y +  viewLine.frame.size.height + Common.Size(s:5), width: UIScreen.main.bounds.size.width / 3 * 2, height: sizeNameString.height))
        ...
        txtChiTietPHBNhan = UILabel(frame: CGRect(x: txtTongPHBNhan.frame.origin.x + txtTongPHBNhan.frame.size.width + 10, y: viewLine.frame.origin.y +  viewLine.frame.size.height + Common.Size(s:5), width: UIScreen.main.bounds.size.width / 3 , height: sizeNameString.height))
        ...
        txtTongPHBGiao = UILabel(frame: CGRect(x: 20, y: txtTongPHBNhan.frame.origin.y +  txtTongPHBNhan.frame.size.height + Common.Size(s:5), width: UIScreen.main.bounds.size.width / 3 * 2, height: sizeNameString.height))
        ...
        txtChiTietPHBGiao = UILabel(frame: CGRect(x: txtTongPHBNhan.frame.origin.x + txtTongPHBNhan.frame.size.width + 10, y: txtTongPHBGiao.frame.origin.y , width: UIScreen.main.bounds.size.width / 3 , height: sizeNameString.height))
        ...
        //////radiobutton
        radioCompany = createRadioButtonPayType(CGRect(x: Common.Size(s:10) ,y: txtTongPHBGiao.frame.size.height + txtTongPHBGiao.frame.origin.y + Common.Size(s:10) , width: frame.size.width / 3, height: Common.Size(s:20)), title: "Nhận hàng", color: UIColor.black);
        ...
        radioMarket = createRadioButtonPayType(CGRect(x: radioCompany.frame.width, y: txtTongPHBGiao.frame.size.height + txtTongPHBGiao.frame.origin.y + Common.Size(s:10), width: frame.size.width / 3, height: Common.Size(s:20)), title: "Giao hàng", color: UIColor.black);
        ...
        radioKhac = createRadioButtonPayType(CGRect(x: radioMarket.frame.width + radioMarket.frame.origin.x, y: txtTongPHBGiao.frame.size.height + txtTongPHBGiao.frame.origin.y + Common.Size(s:10), width: frame.size.width / 3, height: Common.Size(s:20)), title: "Khác", color: UIColor.black);
        //radioMarket.backgroundColor = UIColor(netHex:0x30789e)
        ...
        viewThuKho = UIView(frame: CGRect(x: 0,y: radioMarket.frame.origin.y +  radioMarket.frame.size.height + Common.Size(s:20) ,width:UIScreen.main.bounds.size.width , height: 500))
        viewThuKho.backgroundColor = UIColor(netHex:0xffffff)
        
        
        let txtLableThuKho = UILabel(frame: CGRect(x: Common.Size(s:15), y: Common.Size(s:10), width: scrollView.frame.size.width - Common.Size(s:30), height: Common.Size(s:14)))
        txtLableThuKho.textAlignment = .left
        txtLableThuKho.textColor = UIColor(netHex:0x000000)
        txtLableThuKho.font = UIFont.boldSystemFont(ofSize: Common.Size(s:13))
        txtLableThuKho.text = "Thủ kho xác nhận :"
        viewThuKho.addSubview(txtLableThuKho)
        
        let lbTextThuKhoCode = UILabel(frame: CGRect(x: Common.Size(s:15), y: txtLableThuKho.frame.size.height +  txtLableThuKho.frame.origin.y + Common.Size(s:5), width: scrollView.frame.size.width - Common.Size(s:30), height: Common.Size(s:14)))
        ...
        viewImageSign = UIView(frame: CGRect(x:Common.Size(s:15), y: lbTextSign.frame.origin.y + lbTextSign.frame.size.height + Common.Size(s:5), width: scrollView.frame.size.width - Common.Size(s:30), height: Common.Size(s:100) * 2) )
        ...
        lbTextImage.font = UIFont.systemFont(ofSize: Common.Size(s:14))
        ...
        scrollView.contentSize = CGSize(width:UIScreen.main.bounds.size.width, height: btnXacNhan.frame.origin.y + btnXacNhan.frame.size.height + viewChuKy.frame.size.height + Common.Size(s: 100))
        viewBottom.frame.size.height = btnXacNhan.frame.origin.y + btnXacNhan.frame.size.height + viewChuKy.frame.size.height
    ...
    @objc @IBAction fileprivate func logSelectedButtonBonus(_ radioButton : DLRadioButton) {
        if (radioButton.isMultipleSelectionEnabled) {
            ...
            else if (tag == 3)
            {
                ...
                if(RADIO_CHOSSEN_KHAC == true)
                {
                    self.edtGhiChu.becomeFirstResponder()
                }
