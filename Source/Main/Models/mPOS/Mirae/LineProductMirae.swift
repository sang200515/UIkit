...
import Foundation
import SwiftyJSON
class LineProductMirae: NSObject {
    var Quantity:Int
    var Price:Float
    var Quantity1:Int
    ...
    class func parseObjfromArray(array:[JSON])->[LineProductMirae]{
        var list:[LineProductMirae] = []
        for item in array {
            list.append(self.getObjFromDictionary(data: item))
        }
        return list
    }
    class func getObjFromDictionary(data:JSON) -> LineProductMirae{
        ...
        return LineProductMirae( Quantity:Quantity!
            , Price:Price!
            , Quantity1:Quantity1!
