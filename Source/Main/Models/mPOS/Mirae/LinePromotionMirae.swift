...
import Foundation
import SwiftyJSON
class LinePromotionMirae: NSObject {
    var Quantity:Int
    var Price:Int
    var Quantity1:Int
    ...
    class func parseObjfromArray(array:[JSON])->[LinePromotionMirae]{
        var list:[LinePromotionMirae] = []
        for item in array {
            list.append(self.getObjFromDictionary(data: item))
        }
        return list
    }
    class func getObjFromDictionary(data:JSON) -> LinePromotionMirae{
        ...
        return LinePromotionMirae( Quantity:Quantity!
            , Price:Price!
            , Quantity1:Quantity1!
