...
//  fptshop
//
//  Created by DiemMy Le on 12/7/20.
        ...
        if let vc = self.calendarVC {
            if let dateSelected = vc.getDate() {
                ...
                UIView.animate(withDuration: 0.3, delay: 0, usingSpringWithDamping: 1.0, initialSpringVelocity: 0, options: .curveEaseOut, animations: {
                    ...
                    NotificationCenter.default.post(name: NSNotification.Name.init("didCloseAVCalendar"), object: nil, userInfo: nil)
                }
            } else {
...
class AVHorizontalScrollCollectionView: UICollectionView {
